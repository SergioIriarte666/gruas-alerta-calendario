...
const storagePath = (value: string | null, bucket: string): string | null => {
  ...
  for (
    const marker of [`/object/sign/${bucket}/`, `/object/public/${bucket}/`]
  ) {
...
const sourcesFor = (row: InspectionRow): SourceFile[] => {
  ...
  if (retiro) {
    files.push({ bucket: PDF_BUCKET, sourcePath: retiro, kind: "pdf_retiro" });
  }
...
const keyFor = (serviceId: string, file: SourceFile): string => {
  const segment = file.kind === "photo"
    ? `photos/${file.group}`
    : `pdfs/${file.kind}`;
  const orderedName = file.kind === "photo"
    ? `${file.index ?? 0}-${safeName(file.sourcePath)}`
    : safeName(file.sourcePath);
...
const audit = async (
  ...
  if (auditError) {
    console.error(
      `[archive-inspections-to-r2] auditoría ${row.id}: ${auditError.message}`,
    );
  }
};
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const requestedLimit = Number(
      new URL(req.url).searchParams.get("limit") || DEFAULT_BATCH_LIMIT,
    );
    const limit = Math.min(
      Math.max(
        Number.isFinite(requestedLimit) ? requestedLimit : DEFAULT_BATCH_LIMIT,
        1,
      ),
      MAX_BATCH_LIMIT,
    );
    ...
    const { data, error } = await supabase
      .from("inspections")
      .select(
        "id,service_id,pdf_url,pdf_retiro_url,photos_before_service,photos_client_vehicle,photos_equipment_used,archive_manifest",
      )
      .eq("storage_tier", "hot")
    ...
    for (const row of (data || []) as InspectionRow[]) {
      try {
        ...
        const previous = Array.isArray(row.archive_manifest)
        ...
        for (const source of sources) {
          const staged = previous.find((item) =>
            item.bucket === source.bucket &&
            item.sourcePath === source.sourcePath
          );
          if (
            staged &&
            await headVerifiedObject(
              r2,
              staged.r2Path,
              staged.size,
              staged.sha256,
          ...
          if (downloadError || !blob) {
            throw new Error(
              `No se pudo descargar ${source.bucket}/${source.sourcePath}: ${
        ...
        const photoPaths = (group: PhotoGroup) =>
          manifest
            .filter((item) => item.kind === "photo" && item.group === group)
            .sort((a, b) => (a.index || 0) - (b.index || 0))
            .map((item) => item.r2Path);
        const stagedPaths = {
          r2_pdf_path: manifest.find((item) => item.kind === "pdf")?.r2Path ||
            null,
          r2_pdf_retiro_path: manifest.find((item) =>
            item.kind === "pdf_retiro"
          )?.r2Path || null,
          r2_photos: {
        ...
        if (stageError) {
          throw new Error(
            `No se pudo guardar el manifiesto: ${stageError.message}`,
          );
        }
        ...
        for (const bucket of [PDF_BUCKET, PHOTO_BUCKET]) {
          const paths = [
            ...new Set(
              sources.filter((item) => item.bucket === bucket).map((item) =>
                item.sourcePath
              ),
            ),
          ];
          ...
          if (removeError) {
            throw new Error(
              `No se pudo borrar de ${bucket}: ${removeError.message}`,
            );
          }
        }
        ...
        if (finalizeError) {
          throw new Error(
            `No se pudo finalizar el archivado: ${finalizeError.message}`,
          );
        }
    ...
    const message = errorMessage(cause);
    console.error(`[archive-inspections-to-r2] error general: ${message}`);
