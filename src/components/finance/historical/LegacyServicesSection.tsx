...
import { BarChart3, Check, ChevronDown, CircleDollarSign, Download, FileArchive, Info, Loader2, Search, ShieldCheck, Trash2, Upload, UsersRound } from 'lucide-react';
    ...
    <div className="flex items-center gap-1.5">
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button type="button" className="rounded-full text-muted-foreground transition-colors hover:text-amber-600" aria-label={`Información sobre ${label}`}>
...
const columns: ColumnDef<LegacyServiceRecord>[] = [
  ...
  { accessorKey: 'manual_folio', header: () => <HeaderTooltip label="Folio sistema antiguo" description="Consecutivo interno asignado por la plataforma de membresía anterior. Identifica la fila pero no tiene valor operacional. 100% lleno en la mayoría de los archivos." />, cell: ({ row }) => row.original.manual_folio || '—' },
  { accessorKey: 'expediente', header: () => <HeaderTooltip label="N° Siniestro / Expediente" description="Número del siniestro asignado por la aseguradora cliente (Reale, Axa, Rac, Sur Asistencia, Auxilia, etc). Solo presente cuando aplica." />, cell: ({ row }) => row.original.expediente || '—' },
  ...
  { accessorKey: 'vin', header: 'VIN', cell: ({ row }) => <span className="font-mono text-xs">{row.original.vin || '—'}</span> },
  ...
  { accessorKey: 'observations', header: 'Observaciones', cell: ({ row }) => <p className="max-w-72 truncate" title={row.original.observations || undefined}>{row.original.observations || '—'}</p> },
];
...
export function LegacyServicesSection() {
  ...
  const exportExcel = async () => {
    ...
    try {
      ...
      const sheet = XLSX.utils.json_to_sheet(rows.map((row) => ({ Fecha: formatForDisplayWithTime(row.received_at), Folio: row.manual_folio, Expediente: row.expediente, Aseguradora: row.insurer, 'Tipo de servicio': row.service_type, Marca: row.vehicle_brand, Vehículo: row.vehicle_type, Placa: row.license_plate, VIN: row.vin, Origen: row.origin, Destino: row.destination, Grúa: row.crane_label, Operador: row.operator_label, Subtotal: row.subtotal_clp, Total: row.total_clp, Observaciones: row.observations })));
