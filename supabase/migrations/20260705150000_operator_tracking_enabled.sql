...
COMMENT ON FUNCTION public.get_operator_live_locations() IS
  'Estado de rastreo en vivo por operador activo con tracking_enabled=true: última sesión (cualquier status) + último punto conocido + folio del servicio asociado. SECURITY INVOKER: aplican las policies de admin sobre operator_location_sessions/points.';
