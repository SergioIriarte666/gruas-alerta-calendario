...
serve(async (req) => {
  ...
  try {
    ...
    const { type = 'full', format = 'json' } = body;

    console.log(`Starting ${type} backup generation (${format} format) for user ${user.email}...`);
    ...
    // Start backup log
    const logId = await logger.startLog(`${type}_${format}`, user.id);
    ...
    try {
      ...
      if (format === 'sql') {
        backupResult = await generators.generateSQLBackup(user.email);
      } else if (type === 'quick') {
        backupResult = await generators.generateQuickBackup(user.email);
      } else {
        backupResult = await generators.generateFullBackup(user.email);
      }
