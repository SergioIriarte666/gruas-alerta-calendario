...
const siiRcvRoot = resolve(workspaceRoot, "src/components/siircv");
...
const protectedFiles = collectSourceFiles(siiRcvRoot);
...
describe("contrato visual de SII RCV", () => {
  it.each(protectedFiles)("mantiene estilos semánticos en %s", (file) => {
    ...
    expect(source, `${label} contiene el marcador visual inerte lowboy-on-color`).not.toContain("lowboy-on-color");
  });
});
