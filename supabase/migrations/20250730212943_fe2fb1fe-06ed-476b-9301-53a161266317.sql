-- Corregir trigger para asegurar unit_price positivo
...
BEGIN
  ...
  IF NEW.movement_type = 'exit' AND NEW.crane_id IS NOT NULL THEN
    ...
    INSERT INTO public.crane_parts (
      ...
      -- total_value se calculará automáticamente como (-quantity) * safe_unit_price
    ...
    RAISE NOTICE 'Creado registro de consumo en crane_parts para item: %, crane: %, cantidad: %, precio: %, costo: %', 
      (SELECT name FROM public.inventory_items WHERE id = NEW.item_id), NEW.crane_id, NEW.quantity, safe_unit_price, cost_amount;
  END IF;
