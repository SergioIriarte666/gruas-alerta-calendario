...
-- Crear función para corregir facturas con paid_amount duplicado
CREATE FUNCTION fix_duplicate_paid_amounts()
RETURNS TABLE(
  invoice_id UUID,
  folio TEXT,
  old_paid_amount DECIMAL(10,2),
  correct_paid_amount DECIMAL(10,2),
...
-- Ejecutar corrección inmediata de data
SELECT * FROM fix_duplicate_paid_amounts();
...
BEGIN
  ...
  RAISE NOTICE '✓ Validación exitosa: % triggers activos, % facturas inconsistentes restantes', 
    v_trigger_count, v_inconsistent_invoices;
END $$;
