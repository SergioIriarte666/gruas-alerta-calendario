...
Deno.serve(async (req: Request) => {
  ...
  if (!body.serviceId && !body.inspectionId && !body.folio) {
    return withHeaders(
      jsonResponse(
        { error: "serviceId, inspectionId o folio es requerido" },
        400,
      ),
      cors(req),
    );
  ...
  if (!serviceId && body.folio) {
    ...
    if (serviceError) {
      return withHeaders(
        jsonResponse({ error: serviceError.message }, 500),
        cors(req),
      );
    }
    if (!service) {
  ...
  let query = dataClient
    .from("inspections")
    .select(
      "id,service_id,storage_tier,archived_at,deleted_at,r2_pdf_path,r2_pdf_retiro_path,r2_photos",
    );
  query = body.inspectionId
    ? query.eq("id", body.inspectionId)
    : query.eq("service_id", serviceId!);
  const { data: inspection, error } = await query.maybeSingle();
  if (error) {
    return withHeaders(jsonResponse({ error: error.message }, 500), cors(req));
  }
  if (!inspection) {
    return withHeaders(
      jsonResponse({ error: "Inspección no encontrada" }, 404),
      cors(req),
    );
  }
  ...
  if (inspection.storage_tier !== "cold") {
    return withHeaders(
      jsonResponse({ tier: inspection.storage_tier }),
      cors(req),
    );
  }
  ...
  const sign = async (key: string | null) =>
    key ? await createR2DownloadUrl(r2, key, expiresIn) : null;
  const groups = (inspection.r2_photos || {}) as Record<string, string[]>;
  const signMany = (keys: string[] | undefined) =>
  ...
  try {
    ...
    return withHeaders(
      jsonResponse(
        { error: "El respaldo archivado no está disponible en R2" },
        502,
      ),
      cors(req),
    );
