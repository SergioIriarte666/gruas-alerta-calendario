DROP FUNCTION IF EXISTS public.list_bank_statement_invoice_candidates(uuid);
...
BEGIN
  ...
  WITH candidate_base AS (
    SELECT
      ...
      nullif(
        ltrim(regexp_replace(lower(coalesce(c.rut, '')), '[^0-9k]+', '', 'g'), '0'),
        ''
      ) AS client_rut_compact,
      trim(
        regexp_replace(
      ...
      nullif(
        array_to_string(
          array_remove(
            ARRAY[
              CASE
                WHEN coalesce(array_length(v_detected_client_ruts, 1), 0) > 0
                  AND candidate.client_rut_compact = ANY(v_detected_client_ruts)
                THEN 'RUT cartola coincide con cliente'
                ELSE NULL
              END,
              CASE
                WHEN v_reference_compact IS NOT NULL
  ...
  SELECT
    ...
    scored.match_reason
  FROM scored
  WHERE coalesce(array_length(v_detected_client_ruts, 1), 0) = 0
    OR scored.client_rut_compact = ANY(v_detected_client_ruts)
