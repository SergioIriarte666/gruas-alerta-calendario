-- ============================================================================
-- BASELINE CANDIDATO — esquema public de produccion (jqszxljtfuknhuvuheko)
-- Generado 2026-06-10 via Management API (catalogos pg_*), Fase 1 — REVISION.
-- NO APLICAR sobre produccion. Pensado para reset/replay en entornos limpios.
-- ============================================================================
...
CREATE TYPE public.application_method AS ENUM ('fifo', 'manual', 'proportional');

CREATE TYPE public.closure_status AS ENUM ('open', 'closed', 'invoiced', 'quoted', 'purchase_order_pending');

CREATE TYPE public.crane_status AS ENUM ('active', 'inactive', 'sold', 'written_off');

CREATE TYPE public.crane_type AS ENUM ('light', 'medium', 'heavy', 'taxi', 'other', 'horquilla');

CREATE TYPE public.invoice_status AS ENUM ('draft', 'sent', 'paid', 'overdue', 'cancelled', 'partial');

CREATE TYPE public.payment_status AS ENUM ('pending', 'applied', 'partial', 'cancelled');

CREATE TYPE public.service_status AS ENUM ('pending', 'in_progress', 'completed', 'cancelled', 'invoiced', 'inspection_completed', 'quoted', 'purchase_order_pending', 'with_purchase_order', 'failed');

CREATE TYPE public.supplier_category AS ENUM ('combustible', 'mantenimiento', 'seguros', 'otros', 'peajes', 'salarios', 'administrativos', 'impuestos', 'comision_operador');

CREATE TYPE public.supplier_payment_status AS ENUM ('pending', 'paid', 'overdue', 'cancelled');

-- ===== SECUENCIAS =====
CREATE SEQUENCE IF NOT EXISTS public.audit_log_id_seq START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 CACHE 1;

CREATE SEQUENCE IF NOT EXISTS public.import_batch_records_id_seq START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 CACHE 1;

CREATE SEQUENCE IF NOT EXISTS public.service_cash_receipt_seq START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 CACHE 1;

CREATE SEQUENCE IF NOT EXISTS public.user_activity_log_id_seq START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 CACHE 1;
...
CREATE TABLE public.backup_email_config (
...
CREATE TABLE public.cost_bulk_payment_operations (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.cost_change_history (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.crane_part_change_history (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.cranes (
  ...
  status crane_status DEFAULT 'active'::crane_status NOT NULL
);
...
CREATE TABLE public.import_batch_records (
  id bigint DEFAULT nextval('import_batch_records_id_seq'::regclass) NOT NULL,
...
CREATE TABLE public.import_history_log (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.import_rut_mappings (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  organization_id uuid NOT NULL,
  import_type text NOT NULL,
...
CREATE TABLE public.internal_scheduler_secrets (
  key text NOT NULL,
  value text NOT NULL,
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  updated_at timestamp with time zone DEFAULT now() NOT NULL
...
CREATE TABLE public.inventory_movement_change_history (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  movement_id uuid NOT NULL,
  changed_by uuid,
  changed_at timestamp with time zone DEFAULT now() NOT NULL,
...
CREATE TABLE public.invoice_alert_settings (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.patent_search_history (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  user_id uuid NOT NULL,
...
CREATE TABLE public.payment_applications (
  ...
  application_method application_method DEFAULT 'manual'::application_method,
  notes text,
  created_by uuid,
  created_at timestamp with time zone DEFAULT now()
);
...
CREATE TABLE public.service_cash_receipts (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  service_id uuid NOT NULL,
  folio text DEFAULT generate_service_cash_receipt_folio() NOT NULL,
...
CREATE TABLE public.service_closures (
  ...
  client_id uuid,
  total numeric(10,2) NOT NULL,
  status closure_status DEFAULT 'open'::closure_status,
  created_by uuid,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  purchase_order text
...
CREATE TABLE public.service_update_error_logs (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  service_id uuid,
  user_id uuid,
  error_code text,
...
CREATE TABLE public.services (
  ...
  status service_status DEFAULT 'pending'::service_status,
  observations text,
  created_by uuid,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
...
CREATE TABLE public.suppliers (
  ...
  category text DEFAULT 'otros'::supplier_category NOT NULL,
  created_at timestamp with time zone DEFAULT now(),
...
CREATE TABLE public.vehicle_api_cache (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  endpoint text NOT NULL,
...
CREATE TABLE public.whatsapp_alert_dedupe (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
...
ALTER TABLE public.backup_email_config ADD CONSTRAINT backup_email_config_schedule_hour_check CHECK (((schedule_hour >= 0) AND (schedule_hour <= 23)));

ALTER TABLE public.backup_email_config ADD CONSTRAINT backup_email_config_signed_url_days_check CHECK (((signed_url_days >= 1) AND (signed_url_days <= 30)));

ALTER TABLE public.backup_email_config ADD CONSTRAINT backup_email_config_pkey PRIMARY KEY (id);

ALTER TABLE public.backup_logs ADD CONSTRAINT backup_logs_backup_type_check CHECK ((backup_type = ANY (ARRAY['full'::text, 'quick'::text, 'auto'::text, 'full_sql'::text, 'full_json'::text, 'quick_json'::text])));

ALTER TABLE public.backup_logs ADD CONSTRAINT backup_logs_status_check CHECK ((status = ANY (ARRAY['started'::text, 'completed'::text, 'failed'::text])));

ALTER TABLE public.backup_logs ADD CONSTRAINT backup_logs_pkey PRIMARY KEY (id);

ALTER TABLE public.calendar_events ADD CONSTRAINT calendar_events_status_check CHECK ((status = ANY (ARRAY['scheduled'::text, 'completed'::text, 'cancelled'::text])));

ALTER TABLE public.calendar_events ADD CONSTRAINT calendar_events_type_check CHECK ((type = ANY (ARRAY['service'::text, 'maintenance'::text, 'meeting'::text, 'deadline'::text, 'other'::text])));

ALTER TABLE public.calendar_events ADD CONSTRAINT check_calendar_event_status CHECK ((status = ANY (ARRAY['scheduled'::text, 'completed'::text, 'cancelled'::text])));

ALTER TABLE public.calendar_events ADD CONSTRAINT check_calendar_event_type CHECK ((type = ANY (ARRAY['service'::text, 'maintenance'::text, 'meeting'::text, 'deadline'::text, 'other'::text])));
...
ALTER TABLE public.closure_services ADD CONSTRAINT closure_services_pkey PRIMARY KEY (id);

ALTER TABLE public.closure_services ADD CONSTRAINT closure_services_closure_id_service_id_key UNIQUE (closure_id, service_id);

ALTER TABLE public.company_data ADD CONSTRAINT company_data_pkey PRIMARY KEY (id);

ALTER TABLE public.company_data ADD CONSTRAINT company_data_rut_key UNIQUE (rut);

ALTER TABLE public.company_profiles ADD CONSTRAINT company_profiles_pkey PRIMARY KEY (rut);

ALTER TABLE public.cost_bulk_payment_operations ADD CONSTRAINT cost_bulk_payment_operations_status_check CHECK ((status = ANY (ARRAY['success'::text, 'error'::text])));

ALTER TABLE public.cost_bulk_payment_operations ADD CONSTRAINT cost_bulk_payment_operations_pkey PRIMARY KEY (id);

ALTER TABLE public.cost_categories ADD CONSTRAINT cost_categories_pkey PRIMARY KEY (id);

ALTER TABLE public.cost_categories ADD CONSTRAINT cost_categories_name_key UNIQUE (name);

ALTER TABLE public.cost_centers ADD CONSTRAINT cost_centers_budget_period_check CHECK ((budget_period = ANY (ARRAY['monthly'::text, 'quarterly'::text, 'yearly'::text])));
...
ALTER TABLE public.cost_change_history ADD CONSTRAINT cost_change_history_change_type_check CHECK ((change_type = ANY (ARRAY['CREATE'::text, 'UPDATE'::text, 'DELETE'::text, 'SNAPSHOT'::text])));

ALTER TABLE public.cost_change_history ADD CONSTRAINT cost_change_history_pkey PRIMARY KEY (id);

ALTER TABLE public.cost_inventory_items ADD CONSTRAINT cost_inventory_items_pkey PRIMARY KEY (id);

ALTER TABLE public.cost_inventory_items ADD CONSTRAINT cost_inventory_items_cost_id_inventory_item_id_key UNIQUE (cost_id, inventory_item_id);

ALTER TABLE public.cost_subcategories ADD CONSTRAINT cost_subcategories_pkey PRIMARY KEY (id);

ALTER TABLE public.costs ADD CONSTRAINT costs_amount_check CHECK ((amount > (0)::numeric));

ALTER TABLE public.costs ADD CONSTRAINT costs_pkey PRIMARY KEY (id);

ALTER TABLE public.crane_consumption_rates ADD CONSTRAINT crane_consumption_rates_pkey PRIMARY KEY (id);

ALTER TABLE public.crane_documents ADD CONSTRAINT crane_documents_pkey PRIMARY KEY (id);

ALTER TABLE public.crane_documents ADD CONSTRAINT crane_documents_crane_id_document_type_key UNIQUE (crane_id, document_type);

ALTER TABLE public.crane_maintenance ADD CONSTRAINT crane_maintenance_maintenance_type_check CHECK ((maintenance_type = ANY (ARRAY['preventive'::text, 'corrective'::text, 'emergency'::text])));

ALTER TABLE public.crane_maintenance ADD CONSTRAINT crane_maintenance_status_check CHECK ((status = ANY (ARRAY['scheduled'::text, 'in_progress'::text, 'completed'::text, 'cancelled'::text])));

ALTER TABLE public.crane_maintenance ADD CONSTRAINT crane_maintenance_pkey PRIMARY KEY (id);

ALTER TABLE public.crane_part_change_history ADD CONSTRAINT crane_part_change_history_change_type_check CHECK ((change_type = ANY (ARRAY['CREATE'::text, 'UPDATE'::text, 'DELETE'::text, 'SNAPSHOT'::text])));

ALTER TABLE public.crane_part_change_history ADD CONSTRAINT crane_part_change_history_pkey PRIMARY KEY (id);

ALTER TABLE public.crane_parts ADD CONSTRAINT crane_parts_quantity_check CHECK ((quantity <> 0));

ALTER TABLE public.crane_parts ADD CONSTRAINT crane_parts_unit_price_check CHECK ((unit_price > (0)::numeric));

ALTER TABLE public.crane_parts ADD CONSTRAINT crane_parts_pkey PRIMARY KEY (id);

ALTER TABLE public.cranes ADD CONSTRAINT cranes_pkey PRIMARY KEY (id);

ALTER TABLE public.cranes ADD CONSTRAINT cranes_license_plate_key UNIQUE (license_plate);

ALTER TABLE public.creditors ADD CONSTRAINT creditors_type_check CHECK ((type = ANY (ARRAY['tax'::text, 'supplier'::text, 'other'::text])));

ALTER TABLE public.creditors ADD CONSTRAINT creditors_pkey PRIMARY KEY (id);

ALTER TABLE public.debt_installments ADD CONSTRAINT debt_installments_status_check CHECK ((status = ANY (ARRAY['pending'::text, 'paid'::text])));

ALTER TABLE public.debt_installments ADD CONSTRAINT debt_installments_pkey PRIMARY KEY (id);

ALTER TABLE public.debt_installments ADD CONSTRAINT debt_installments_unique_debt_number UNIQUE (debt_id, installment_number);
...
ALTER TABLE public.debts ADD CONSTRAINT debts_frequency_check CHECK ((frequency = 'monthly'::text));

ALTER TABLE public.debts ADD CONSTRAINT debts_installments_count_check CHECK ((installments_count > 0));

ALTER TABLE public.debts ADD CONSTRAINT debts_status_check CHECK ((status = ANY (ARRAY['active'::text, 'paid'::text, 'cancelled'::text])));

ALTER TABLE public.debts ADD CONSTRAINT debts_pkey PRIMARY KEY (id);

ALTER TABLE public.document_alerts ADD CONSTRAINT document_alerts_document_type_check CHECK ((document_type = ANY (ARRAY['technical_review'::text, 'insurance'::text, 'circulation_permit'::text])));

ALTER TABLE public.document_alerts ADD CONSTRAINT document_alerts_pkey PRIMARY KEY (id);

ALTER TABLE public.document_alerts ADD CONSTRAINT document_alerts_crane_id_document_type_key UNIQUE (crane_id, document_type);
...
ALTER TABLE public.import_batch_records ADD CONSTRAINT import_batch_records_pkey PRIMARY KEY (id);

ALTER TABLE public.import_batches ADD CONSTRAINT import_batches_pkey PRIMARY KEY (id);

ALTER TABLE public.import_history_log ADD CONSTRAINT import_history_log_import_type_check CHECK ((import_type = ANY (ARRAY['purchase'::text, 'sale'::text])));

ALTER TABLE public.import_history_log ADD CONSTRAINT import_history_log_status_check CHECK ((status = ANY (ARRAY['success'::text, 'partial'::text, 'failed'::text])));

ALTER TABLE public.import_history_log ADD CONSTRAINT import_history_log_pkey PRIMARY KEY (id);

ALTER TABLE public.import_rut_mappings ADD CONSTRAINT import_rut_mappings_import_type_check CHECK ((import_type = ANY (ARRAY['purchase'::text, 'sale'::text])));

ALTER TABLE public.import_rut_mappings ADD CONSTRAINT import_rut_mappings_resolution_check CHECK ((resolution = ANY (ARRAY['create'::text, 'assign'::text, 'ignore'::text])));

ALTER TABLE public.import_rut_mappings ADD CONSTRAINT import_rut_mappings_pkey PRIMARY KEY (id);

ALTER TABLE public.import_rut_mappings ADD CONSTRAINT import_rut_mappings_organization_id_import_type_source_rut_key UNIQUE (organization_id, import_type, source_rut);

ALTER TABLE public.income_categories ADD CONSTRAINT income_categories_pkey PRIMARY KEY (id);

ALTER TABLE public.income_categories ADD CONSTRAINT income_categories_name_key UNIQUE (name);

ALTER TABLE public.income_subcategories ADD CONSTRAINT income_subcategories_pkey PRIMARY KEY (id);
...
ALTER TABLE public.internal_scheduler_secrets ADD CONSTRAINT internal_scheduler_secrets_pkey PRIMARY KEY (key);

ALTER TABLE public.inventory_alerts ADD CONSTRAINT inventory_alerts_alert_type_check CHECK ((alert_type = ANY (ARRAY['low_stock'::text, 'expiring_soon'::text, 'overstock'::text, 'no_movement'::text, 'excess_stock'::text, 'expiration'::text])));
...
ALTER TABLE public.inventory_categories ADD CONSTRAINT inventory_categories_code_key UNIQUE (code);

ALTER TABLE public.inventory_consumptions ADD CONSTRAINT inventory_consumptions_maintenance_type_check CHECK ((maintenance_type = ANY (ARRAY['preventive'::text, 'corrective'::text, 'emergency'::text])));
...
ALTER TABLE public.inventory_locations ADD CONSTRAINT inventory_locations_code_key UNIQUE (code);

ALTER TABLE public.inventory_movement_change_history ADD CONSTRAINT inventory_movement_change_history_change_type_check CHECK ((change_type = ANY (ARRAY['CREATE'::text, 'UPDATE'::text, 'DELETE'::text, 'SNAPSHOT'::text])));

ALTER TABLE public.inventory_movement_change_history ADD CONSTRAINT inventory_movement_change_history_pkey PRIMARY KEY (id);

ALTER TABLE public.inventory_movements ADD CONSTRAINT inventory_movements_movement_type_check CHECK ((movement_type = ANY (ARRAY['entry'::text, 'exit'::text, 'transfer'::text, 'adjustment'::text, 'sale'::text])));

ALTER TABLE public.inventory_movements ADD CONSTRAINT inventory_movements_status_check CHECK ((status = ANY (ARRAY['active'::text, 'cancelled'::text])));
...
ALTER TABLE public.inventory_stock ADD CONSTRAINT unique_item_location UNIQUE (item_id, location_id);

ALTER TABLE public.inventory_suppliers ADD CONSTRAINT inventory_suppliers_pkey PRIMARY KEY (id);

ALTER TABLE public.invoice_alert_settings ADD CONSTRAINT invoice_alert_settings_pkey PRIMARY KEY (id);

ALTER TABLE public.invoice_cancellations ADD CONSTRAINT invoice_cancellations_pkey PRIMARY KEY (id);

ALTER TABLE public.invoice_cancellations ADD CONSTRAINT unique_credit_note_number UNIQUE (credit_note_number);

ALTER TABLE public.invoice_cancellations ADD CONSTRAINT unique_invoice_cancellation UNIQUE (invoice_id);

ALTER TABLE public.invoice_closures ADD CONSTRAINT invoice_closures_pkey PRIMARY KEY (id);

ALTER TABLE public.invoice_services ADD CONSTRAINT invoice_services_pkey PRIMARY KEY (id);

ALTER TABLE public.invoice_services ADD CONSTRAINT invoice_services_invoice_id_service_id_key UNIQUE (invoice_id, service_id);

ALTER TABLE public.invoices ADD CONSTRAINT invoices_paid_amount_check CHECK ((paid_amount >= (0)::numeric));

ALTER TABLE public.invoices ADD CONSTRAINT invoices_product_service_description_len_chk CHECK (((char_length(btrim(product_service_description)) >= 10) AND (char_length(btrim(product_service_description)) <= 500)));

ALTER TABLE public.invoices ADD CONSTRAINT invoices_pkey PRIMARY KEY (id);

ALTER TABLE public.invoices ADD CONSTRAINT invoices_folio_key UNIQUE (folio);

ALTER TABLE public.invoices ADD CONSTRAINT invoices_numero_fiscal_unique UNIQUE (numero_fiscal);

ALTER TABLE public.notification_logs ADD CONSTRAINT notification_logs_status_check CHECK ((status = ANY (ARRAY['pending'::text, 'sent'::text, 'failed'::text])));

ALTER TABLE public.notification_logs ADD CONSTRAINT notification_logs_type_check CHECK ((type = ANY (ARRAY['push'::text, 'email'::text, 'in_app'::text, 'inventory_alert'::text])));
...
ALTER TABLE public.notifications ADD CONSTRAINT notifications_priority_check CHECK (((priority >= 1) AND (priority <= 5)));

ALTER TABLE public.notifications ADD CONSTRAINT notifications_pkey PRIMARY KEY (id);

ALTER TABLE public.operator_documents ADD CONSTRAINT operator_documents_document_type_check CHECK ((document_type = ANY (ARRAY['cedula_identidad'::text, 'licencia_conducir'::text, 'examen_psicosensotecnico'::text, 'examen_altura'::text, 'seguro_vida'::text, 'contrato_trabajo'::text])));

ALTER TABLE public.operator_documents ADD CONSTRAINT operator_documents_pkey PRIMARY KEY (id);

ALTER TABLE public.operator_documents ADD CONSTRAINT operator_documents_operator_id_document_type_key UNIQUE (operator_id, document_type);

ALTER TABLE public.operators ADD CONSTRAINT operators_operator_type_check CHECK ((operator_type = ANY (ARRAY['crane_operator'::text, 'administrative'::text])));

ALTER TABLE public.operators ADD CONSTRAINT operators_pkey PRIMARY KEY (id);

ALTER TABLE public.operators ADD CONSTRAINT operators_license_number_key UNIQUE (license_number);

ALTER TABLE public.operators ADD CONSTRAINT operators_rut_key UNIQUE (rut);

ALTER TABLE public.operators ADD CONSTRAINT operators_user_id_key UNIQUE (user_id);

ALTER TABLE public.password_reset_rate_limits ADD CONSTRAINT password_reset_rate_limits_attempt_count_check CHECK ((attempt_count >= 0));

ALTER TABLE public.password_reset_rate_limits ADD CONSTRAINT password_reset_rate_limits_pkey PRIMARY KEY (email_hash, ip_hash);

ALTER TABLE public.patent_search_history ADD CONSTRAINT patent_search_history_pkey PRIMARY KEY (id);

ALTER TABLE public.payment_applications ADD CONSTRAINT payment_applications_applied_amount_check CHECK ((applied_amount > (0)::numeric));

ALTER TABLE public.payment_applications ADD CONSTRAINT payment_applications_pkey PRIMARY KEY (id);

ALTER TABLE public.payment_applications ADD CONSTRAINT payment_applications_payment_id_invoice_id_key UNIQUE (payment_id, invoice_id);

ALTER TABLE public.payment_terms ADD CONSTRAINT payment_terms_pkey PRIMARY KEY (id);

ALTER TABLE public.payment_terms ADD CONSTRAINT payment_terms_code_key UNIQUE (code);

ALTER TABLE public.payments ADD CONSTRAINT payments_amount_check CHECK ((amount > (0)::numeric));

ALTER TABLE public.payments ADD CONSTRAINT payments_applied_amount_check CHECK ((applied_amount >= (0)::numeric));

ALTER TABLE public.payments ADD CONSTRAINT payments_pkey PRIMARY KEY (id);

ALTER TABLE public.payments ADD CONSTRAINT payments_unique_bank_reference_per_client UNIQUE (client_id, bank_reference, payment_date);

ALTER TABLE public.profiles ADD CONSTRAINT profiles_status_check CHECK ((status = ANY (ARRAY['pending'::text, 'approved'::text, 'rejected'::text])));

ALTER TABLE public.profiles ADD CONSTRAINT profiles_pkey PRIMARY KEY (id);

ALTER TABLE public.profiles ADD CONSTRAINT profiles_email_unique UNIQUE (email);

ALTER TABLE public.purchase_voids ADD CONSTRAINT purchase_voids_pkey PRIMARY KEY (id);

ALTER TABLE public.push_subscriptions ADD CONSTRAINT push_subscriptions_pkey PRIMARY KEY (id);

ALTER TABLE public.push_subscriptions ADD CONSTRAINT push_subscriptions_user_id_key UNIQUE (user_id);

ALTER TABLE public.quick_entries ADD CONSTRAINT quick_entries_status_check CHECK ((status = ANY (ARRAY['pending'::text, 'completed'::text, 'discarded'::text])));

ALTER TABLE public.quick_entries ADD CONSTRAINT quick_entries_type_check CHECK ((type = ANY (ARRAY['service'::text, 'cost'::text, 'inventory'::text, 'maintenance'::text])));

ALTER TABLE public.quick_entries ADD CONSTRAINT quick_entries_pkey PRIMARY KEY (id);

ALTER TABLE public.route_tolls ADD CONSTRAINT route_tolls_pkey PRIMARY KEY (id);

ALTER TABLE public.routes ADD CONSTRAINT routes_pkey PRIMARY KEY (id);

ALTER TABLE public.saved_locations ADD CONSTRAINT saved_locations_pkey PRIMARY KEY (id);

ALTER TABLE public.scheduled_payments ADD CONSTRAINT scheduled_payments_payment_method_check CHECK (((payment_method)::text = ANY ((ARRAY['transfer'::character varying, 'check'::character varying, 'cash'::character varying, 'credit'::character varying])::text[])));

ALTER TABLE public.scheduled_payments ADD CONSTRAINT scheduled_payments_priority_check CHECK (((priority)::text = ANY ((ARRAY['low'::character varying, 'medium'::character varying, 'high'::character varying, 'urgent'::character varying])::text[])));

ALTER TABLE public.scheduled_payments ADD CONSTRAINT scheduled_payments_status_check CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'confirmed'::character varying, 'paid'::character varying, 'cancelled'::character varying, 'rescheduled'::character varying])::text[])));

ALTER TABLE public.scheduled_payments ADD CONSTRAINT scheduled_payments_pkey PRIMARY KEY (id);

ALTER TABLE public.service_cash_receipts ADD CONSTRAINT service_cash_receipts_pkey PRIMARY KEY (id);

ALTER TABLE public.service_cash_receipts ADD CONSTRAINT service_cash_receipts_folio_key UNIQUE (folio);

ALTER TABLE public.service_cash_receipts ADD CONSTRAINT service_cash_receipts_service_id_key UNIQUE (service_id);

ALTER TABLE public.service_change_history ADD CONSTRAINT service_change_history_change_type_check CHECK ((change_type = ANY (ARRAY['CREATE'::text, 'UPDATE'::text, 'DELETE'::text, 'SNAPSHOT'::text])));

ALTER TABLE public.service_change_history ADD CONSTRAINT service_change_history_pkey PRIMARY KEY (id);

ALTER TABLE public.service_closures ADD CONSTRAINT service_closures_pkey PRIMARY KEY (id);

ALTER TABLE public.service_closures ADD CONSTRAINT service_closures_folio_key UNIQUE (folio);

ALTER TABLE public.service_costs ADD CONSTRAINT service_costs_cost_type_check CHECK ((cost_type = ANY (ARRAY['commission'::text, 'fuel'::text, 'maintenance'::text, 'transport'::text, 'other'::text])));

ALTER TABLE public.service_costs ADD CONSTRAINT service_costs_pkey PRIMARY KEY (id);

ALTER TABLE public.service_rates ADD CONSTRAINT service_rates_pkey PRIMARY KEY (id);

ALTER TABLE public.service_resources ADD CONSTRAINT service_resources_resource_type_check CHECK ((resource_type = ANY (ARRAY['operator'::text, 'crane'::text])));

ALTER TABLE public.service_resources ADD CONSTRAINT service_resources_pkey PRIMARY KEY (id);
...
ALTER TABLE public.service_update_error_logs ADD CONSTRAINT service_update_error_logs_pkey PRIMARY KEY (id);

ALTER TABLE public.services ADD CONSTRAINT services_custody_discount_percentage_check CHECK (((custody_discount_percentage >= (0)::numeric) AND (custody_discount_percentage <= (100)::numeric)));

ALTER TABLE public.services ADD CONSTRAINT services_custody_mode_check CHECK ((custody_mode = ANY (ARRAY['manual'::text, 'calendar'::text, 'none'::text])));

ALTER TABLE public.services ADD CONSTRAINT services_service_relationship_type_check CHECK ((service_relationship_type = ANY (ARRAY['main'::text, 'excess'::text])));

ALTER TABLE public.services ADD CONSTRAINT services_urgency_check CHECK ((urgency = ANY (ARRAY['normal'::text, 'urgent'::text])));
...
ALTER TABLE public.supplier_categories ADD CONSTRAINT supplier_categories_pkey PRIMARY KEY (id);

ALTER TABLE public.supplier_categories ADD CONSTRAINT supplier_categories_name_key UNIQUE (name);

ALTER TABLE public.supplier_invoice_items ADD CONSTRAINT supplier_invoice_items_quantity_check CHECK ((quantity > 0));

ALTER TABLE public.supplier_invoice_items ADD CONSTRAINT supplier_invoice_items_subtotal_check CHECK ((subtotal >= (0)::numeric));

ALTER TABLE public.supplier_invoice_items ADD CONSTRAINT supplier_invoice_items_tax_amount_check CHECK ((tax_amount >= (0)::numeric));

ALTER TABLE public.supplier_invoice_items ADD CONSTRAINT supplier_invoice_items_total_amount_check CHECK ((total_amount >= (0)::numeric));

ALTER TABLE public.supplier_invoice_items ADD CONSTRAINT supplier_invoice_items_unit_price_check CHECK ((unit_price >= (0)::numeric));

ALTER TABLE public.supplier_invoice_items ADD CONSTRAINT supplier_invoice_items_pkey PRIMARY KEY (id);

ALTER TABLE public.supplier_invoice_items ADD CONSTRAINT supplier_invoice_items_supplier_invoice_id_line_number_key UNIQUE (supplier_invoice_id, line_number);

ALTER TABLE public.supplier_invoices ADD CONSTRAINT supplier_invoices_product_service_description_len_chk CHECK (((char_length(btrim(product_service_description)) >= 10) AND (char_length(btrim(product_service_description)) <= 500)));

ALTER TABLE public.supplier_invoices ADD CONSTRAINT supplier_invoices_status_check CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'scheduled'::character varying, 'paid'::character varying, 'overdue'::character varying, 'cancelled'::character varying])::text[])));
...
ALTER TABLE public.supplier_payments ADD CONSTRAINT supplier_payments_status_check CHECK ((status = ANY (ARRAY['pending'::text, 'paid'::text, 'overdue'::text, 'cancelled'::text])));

ALTER TABLE public.supplier_payments ADD CONSTRAINT supplier_payments_pkey PRIMARY KEY (id);

ALTER TABLE public.suppliers ADD CONSTRAINT suppliers_pkey PRIMARY KEY (id);

ALTER TABLE public.suppliers ADD CONSTRAINT suppliers_rut_key UNIQUE (rut);

ALTER TABLE public.system_settings ADD CONSTRAINT check_session_timeout_minutes CHECK (((session_timeout_minutes >= 10) AND (session_timeout_minutes <= 120)));

ALTER TABLE public.system_settings ADD CONSTRAINT check_session_warning_minutes CHECK (((session_warning_minutes >= 5) AND (session_warning_minutes <= 60)));

ALTER TABLE public.system_settings ADD CONSTRAINT system_settings_backup_frequency_check CHECK ((backup_frequency = ANY (ARRAY['daily'::text, 'weekly'::text, 'monthly'::text])));

ALTER TABLE public.system_settings ADD CONSTRAINT system_settings_data_retention_check CHECK ((data_retention > 0));

ALTER TABLE public.system_settings ADD CONSTRAINT system_settings_pkey PRIMARY KEY (id);

ALTER TABLE public.toll_rates ADD CONSTRAINT toll_rates_pkey PRIMARY KEY (id);

ALTER TABLE public.toll_stations ADD CONSTRAINT toll_stations_pkey PRIMARY KEY (id);

ALTER TABLE public.trip_estimates ADD CONSTRAINT trip_estimates_pkey PRIMARY KEY (id);

ALTER TABLE public.user_activity_log ADD CONSTRAINT user_activity_log_pkey PRIMARY KEY (id);

ALTER TABLE public.user_invitations ADD CONSTRAINT user_invitations_status_check CHECK ((status = ANY (ARRAY['pending'::text, 'sent'::text, 'accepted'::text, 'expired'::text])));

ALTER TABLE public.user_invitations ADD CONSTRAINT user_invitations_pkey PRIMARY KEY (id);

ALTER TABLE public.user_invitations ADD CONSTRAINT user_invitations_user_id_key UNIQUE (user_id);

ALTER TABLE public.user_module_permissions ADD CONSTRAINT user_module_permissions_pkey PRIMARY KEY (id);

ALTER TABLE public.user_module_permissions ADD CONSTRAINT user_module_permissions_user_id_module_key_key UNIQUE (user_id, module_key);
...
ALTER TABLE public.vehicle_api_cache ADD CONSTRAINT vehicle_api_cache_pkey PRIMARY KEY (id);

ALTER TABLE public.vehicle_api_cache ADD CONSTRAINT vehicle_api_cache_endpoint_value_key UNIQUE (endpoint, lookup_value);
...
ALTER TABLE public.vehicle_models ADD CONSTRAINT vehicle_models_brand_id_name_key UNIQUE (brand_id, name);

ALTER TABLE public.whatsapp_alert_dedupe ADD CONSTRAINT whatsapp_alert_dedupe_pkey PRIMARY KEY (id);

ALTER TABLE public.whatsapp_alert_dedupe ADD CONSTRAINT whatsapp_alert_dedupe_alert_key_sent_for_date_key UNIQUE (alert_key, sent_for_date);

ALTER TABLE public.whatsapp_message_log ADD CONSTRAINT whatsapp_message_log_direction_check CHECK ((direction = ANY (ARRAY['outbound'::text, 'inbound'::text])));

ALTER TABLE public.whatsapp_message_log ADD CONSTRAINT whatsapp_message_log_status_check CHECK ((status = ANY (ARRAY['queued'::text, 'sent'::text, 'delivered'::text, 'read'::text, 'failed'::text])));

ALTER TABLE public.whatsapp_message_log ADD CONSTRAINT whatsapp_message_log_pkey PRIMARY KEY (id);

ALTER TABLE public.whatsapp_settings ADD CONSTRAINT whatsapp_settings_pkey PRIMARY KEY (id);

-- ===== FOREIGN KEYS =====
ALTER TABLE public.audit_log ADD CONSTRAINT audit_log_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id);

ALTER TABLE public.backup_logs ADD CONSTRAINT backup_logs_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);
...
ALTER TABLE public.calendar_events ADD CONSTRAINT calendar_events_crane_id_fkey FOREIGN KEY (crane_id) REFERENCES cranes(id) ON DELETE SET NULL;

ALTER TABLE public.calendar_events ADD CONSTRAINT calendar_events_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id) ON DELETE SET NULL;

ALTER TABLE public.calendar_events ADD CONSTRAINT calendar_events_operator_id_fkey FOREIGN KEY (operator_id) REFERENCES operators(id) ON DELETE SET NULL;

ALTER TABLE public.calendar_events ADD CONSTRAINT calendar_events_service_id_fkey FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE SET NULL;

ALTER TABLE public.clients ADD CONSTRAINT clients_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.clients ADD CONSTRAINT clients_default_payment_term_id_fkey FOREIGN KEY (default_payment_term_id) REFERENCES payment_terms(id);

ALTER TABLE public.closure_services ADD CONSTRAINT closure_services_closure_id_fkey FOREIGN KEY (closure_id) REFERENCES service_closures(id) ON DELETE CASCADE;

ALTER TABLE public.closure_services ADD CONSTRAINT closure_services_service_id_fkey FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE CASCADE;

ALTER TABLE public.cost_bulk_payment_operations ADD CONSTRAINT cost_bulk_payment_operations_executed_by_fkey FOREIGN KEY (executed_by) REFERENCES auth.users(id) ON DELETE CASCADE;

ALTER TABLE public.cost_categories ADD CONSTRAINT cost_categories_default_cost_center_id_fkey FOREIGN KEY (default_cost_center_id) REFERENCES cost_centers(id) ON DELETE SET NULL;

ALTER TABLE public.cost_centers ADD CONSTRAINT cost_centers_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id) ON DELETE SET NULL;
...
ALTER TABLE public.cost_change_history ADD CONSTRAINT cost_change_history_changed_by_profile_fkey FOREIGN KEY (changed_by) REFERENCES profiles(id) ON DELETE SET NULL;

ALTER TABLE public.cost_inventory_items ADD CONSTRAINT cost_inventory_items_cost_id_fkey FOREIGN KEY (cost_id) REFERENCES costs(id) ON DELETE CASCADE;

ALTER TABLE public.cost_inventory_items ADD CONSTRAINT cost_inventory_items_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.cost_inventory_items ADD CONSTRAINT cost_inventory_items_inventory_item_id_fkey FOREIGN KEY (inventory_item_id) REFERENCES inventory_items(id) ON DELETE CASCADE;

ALTER TABLE public.cost_subcategories ADD CONSTRAINT cost_subcategories_category_id_fkey FOREIGN KEY (category_id) REFERENCES cost_categories(id) ON DELETE CASCADE;

ALTER TABLE public.cost_subcategories ADD CONSTRAINT cost_subcategories_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.costs ADD CONSTRAINT costs_category_id_fkey FOREIGN KEY (category_id) REFERENCES cost_categories(id) ON DELETE RESTRICT;

ALTER TABLE public.costs ADD CONSTRAINT costs_cost_center_id_fkey FOREIGN KEY (cost_center_id) REFERENCES cost_centers(id) ON DELETE SET NULL;

ALTER TABLE public.costs ADD CONSTRAINT costs_crane_id_fkey FOREIGN KEY (crane_id) REFERENCES cranes(id) ON DELETE SET NULL;

ALTER TABLE public.costs ADD CONSTRAINT costs_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id) ON DELETE SET NULL;

ALTER TABLE public.costs ADD CONSTRAINT costs_inventory_movement_id_fkey FOREIGN KEY (inventory_movement_id) REFERENCES inventory_movements(id);

ALTER TABLE public.costs ADD CONSTRAINT costs_maintenance_id_fkey FOREIGN KEY (maintenance_id) REFERENCES crane_maintenance(id);

ALTER TABLE public.costs ADD CONSTRAINT costs_operator_id_fkey FOREIGN KEY (operator_id) REFERENCES operators(id) ON DELETE SET NULL;

ALTER TABLE public.costs ADD CONSTRAINT costs_service_id_fkey FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE SET NULL;

ALTER TABLE public.costs ADD CONSTRAINT costs_supplier_id_fkey FOREIGN KEY (supplier_id) REFERENCES inventory_suppliers(id);

ALTER TABLE public.costs ADD CONSTRAINT costs_supplier_invoice_id_fkey FOREIGN KEY (supplier_invoice_id) REFERENCES supplier_invoices(id) ON DELETE SET NULL;

ALTER TABLE public.crane_documents ADD CONSTRAINT crane_documents_crane_id_fkey FOREIGN KEY (crane_id) REFERENCES cranes(id) ON DELETE CASCADE;

ALTER TABLE public.crane_documents ADD CONSTRAINT crane_documents_uploaded_by_fkey FOREIGN KEY (uploaded_by) REFERENCES profiles(id);

ALTER TABLE public.crane_maintenance ADD CONSTRAINT crane_maintenance_crane_id_fkey FOREIGN KEY (crane_id) REFERENCES cranes(id) ON DELETE CASCADE;

ALTER TABLE public.crane_maintenance ADD CONSTRAINT crane_maintenance_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.crane_part_change_history ADD CONSTRAINT crane_part_change_history_changed_by_profile_fkey FOREIGN KEY (changed_by) REFERENCES profiles(id) ON DELETE SET NULL;

ALTER TABLE public.crane_parts ADD CONSTRAINT crane_parts_crane_id_fkey FOREIGN KEY (crane_id) REFERENCES cranes(id) ON DELETE CASCADE;

ALTER TABLE public.crane_parts ADD CONSTRAINT crane_parts_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.crane_parts ADD CONSTRAINT crane_parts_inventory_movement_id_fkey FOREIGN KEY (inventory_movement_id) REFERENCES inventory_movements(id);

ALTER TABLE public.crane_parts ADD CONSTRAINT crane_parts_supplier_id_fkey FOREIGN KEY (supplier_id) REFERENCES inventory_suppliers(id);

ALTER TABLE public.crane_parts ADD CONSTRAINT fk_crane_parts_cost_id FOREIGN KEY (cost_id) REFERENCES costs(id) ON DELETE CASCADE;

ALTER TABLE public.cranes ADD CONSTRAINT cranes_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.creditors ADD CONSTRAINT creditors_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.creditors ADD CONSTRAINT creditors_supplier_id_fkey FOREIGN KEY (supplier_id) REFERENCES inventory_suppliers(id);

ALTER TABLE public.creditors ADD CONSTRAINT creditors_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id);

ALTER TABLE public.debt_installments ADD CONSTRAINT debt_installments_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.debt_installments ADD CONSTRAINT debt_installments_debt_id_fkey FOREIGN KEY (debt_id) REFERENCES debts(id) ON DELETE CASCADE;

ALTER TABLE public.debt_installments ADD CONSTRAINT debt_installments_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id);

ALTER TABLE public.debt_payments ADD CONSTRAINT debt_payments_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.debt_payments ADD CONSTRAINT debt_payments_debt_installment_id_fkey FOREIGN KEY (debt_installment_id) REFERENCES debt_installments(id) ON DELETE CASCADE;

ALTER TABLE public.debt_payments ADD CONSTRAINT debt_payments_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id);

ALTER TABLE public.debts ADD CONSTRAINT debts_cost_center_id_fkey FOREIGN KEY (cost_center_id) REFERENCES cost_centers(id) ON DELETE SET NULL;

ALTER TABLE public.debts ADD CONSTRAINT debts_crane_id_fkey FOREIGN KEY (crane_id) REFERENCES cranes(id) ON DELETE SET NULL;

ALTER TABLE public.debts ADD CONSTRAINT debts_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.debts ADD CONSTRAINT debts_creditor_id_fkey FOREIGN KEY (creditor_id) REFERENCES creditors(id) ON DELETE RESTRICT;

ALTER TABLE public.debts ADD CONSTRAINT debts_operator_id_fkey FOREIGN KEY (operator_id) REFERENCES operators(id) ON DELETE SET NULL;

ALTER TABLE public.debts ADD CONSTRAINT debts_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id);

ALTER TABLE public.document_alerts ADD CONSTRAINT document_alerts_crane_id_fkey FOREIGN KEY (crane_id) REFERENCES cranes(id) ON DELETE CASCADE;
...
ALTER TABLE public.fuel_prices ADD CONSTRAINT fuel_prices_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id);

ALTER TABLE public.import_batch_records ADD CONSTRAINT import_batch_records_batch_id_fkey FOREIGN KEY (batch_id) REFERENCES import_batches(id) ON DELETE CASCADE;
...
ALTER TABLE public.import_batches ADD CONSTRAINT import_batches_rolled_back_by_fkey FOREIGN KEY (rolled_back_by) REFERENCES auth.users(id);

ALTER TABLE public.import_history_log ADD CONSTRAINT import_history_log_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES auth.users(id) ON DELETE CASCADE;

ALTER TABLE public.import_rut_mappings ADD CONSTRAINT import_rut_mappings_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES auth.users(id) ON DELETE CASCADE;

ALTER TABLE public.income_subcategories ADD CONSTRAINT income_subcategories_category_id_fkey FOREIGN KEY (category_id) REFERENCES income_categories(id) ON DELETE CASCADE;

ALTER TABLE public.income_subcategories ADD CONSTRAINT income_subcategories_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.incomes ADD CONSTRAINT incomes_category_id_fkey FOREIGN KEY (category_id) REFERENCES income_categories(id) ON DELETE SET NULL;

ALTER TABLE public.incomes ADD CONSTRAINT incomes_client_id_fkey FOREIGN KEY (client_id) REFERENCES clients(id) ON DELETE SET NULL;

ALTER TABLE public.incomes ADD CONSTRAINT incomes_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.incomes ADD CONSTRAINT incomes_invoice_id_fkey FOREIGN KEY (invoice_id) REFERENCES invoices(id) ON DELETE SET NULL;

ALTER TABLE public.inspections ADD CONSTRAINT inspections_operator_id_fkey FOREIGN KEY (operator_id) REFERENCES operators(id);

ALTER TABLE public.inspections ADD CONSTRAINT inspections_service_id_fkey FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE CASCADE;

ALTER TABLE public.inventory_alerts ADD CONSTRAINT inventory_alerts_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.inventory_alerts ADD CONSTRAINT inventory_alerts_item_id_fkey FOREIGN KEY (item_id) REFERENCES inventory_items(id);

ALTER TABLE public.inventory_alerts ADD CONSTRAINT inventory_alerts_location_id_fkey FOREIGN KEY (location_id) REFERENCES inventory_locations(id);

ALTER TABLE public.inventory_categories ADD CONSTRAINT inventory_categories_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.inventory_consumptions ADD CONSTRAINT inventory_consumptions_approved_by_fkey FOREIGN KEY (approved_by) REFERENCES profiles(id);

ALTER TABLE public.inventory_consumptions ADD CONSTRAINT inventory_consumptions_cost_center_id_fkey FOREIGN KEY (cost_center_id) REFERENCES cost_centers(id);

ALTER TABLE public.inventory_consumptions ADD CONSTRAINT inventory_consumptions_crane_id_fkey FOREIGN KEY (crane_id) REFERENCES cranes(id);

ALTER TABLE public.inventory_consumptions ADD CONSTRAINT inventory_consumptions_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.inventory_consumptions ADD CONSTRAINT inventory_consumptions_movement_id_fkey FOREIGN KEY (movement_id) REFERENCES inventory_movements(id);

ALTER TABLE public.inventory_consumptions ADD CONSTRAINT inventory_consumptions_operator_id_fkey FOREIGN KEY (operator_id) REFERENCES operators(id);
...
ALTER TABLE public.inventory_locations ADD CONSTRAINT inventory_locations_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.inventory_movement_change_history ADD CONSTRAINT inventory_movement_change_history_changed_by_profile_fkey FOREIGN KEY (changed_by) REFERENCES profiles(id) ON DELETE SET NULL;

ALTER TABLE public.inventory_movements ADD CONSTRAINT inventory_movements_cost_id_fkey FOREIGN KEY (cost_id) REFERENCES costs(id) ON DELETE SET NULL;

ALTER TABLE public.inventory_movements ADD CONSTRAINT inventory_movements_crane_id_fkey FOREIGN KEY (crane_id) REFERENCES cranes(id);
...
ALTER TABLE public.inventory_movements ADD CONSTRAINT inventory_movements_location_id_fkey FOREIGN KEY (location_id) REFERENCES inventory_locations(id);

ALTER TABLE public.inventory_movements ADD CONSTRAINT inventory_movements_maintenance_id_fkey FOREIGN KEY (maintenance_id) REFERENCES crane_maintenance(id);

ALTER TABLE public.inventory_movements ADD CONSTRAINT inventory_movements_operator_id_fkey FOREIGN KEY (operator_id) REFERENCES operators(id);

ALTER TABLE public.inventory_movements ADD CONSTRAINT inventory_movements_supplier_id_fkey FOREIGN KEY (supplier_id) REFERENCES inventory_suppliers(id);

ALTER TABLE public.inventory_movements ADD CONSTRAINT inventory_movements_supplier_invoice_id_fkey FOREIGN KEY (supplier_invoice_id) REFERENCES supplier_invoices(id) ON DELETE SET NULL;

ALTER TABLE public.inventory_movements ADD CONSTRAINT inventory_movements_supplier_invoice_item_id_fkey FOREIGN KEY (supplier_invoice_item_id) REFERENCES supplier_invoice_items(id) ON DELETE SET NULL;
...
ALTER TABLE public.inventory_suppliers ADD CONSTRAINT inventory_suppliers_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.inventory_suppliers ADD CONSTRAINT inventory_suppliers_default_payment_term_id_fkey FOREIGN KEY (default_payment_term_id) REFERENCES payment_terms(id);

ALTER TABLE public.invoice_cancellations ADD CONSTRAINT invoice_cancellations_cancelled_by_fkey FOREIGN KEY (cancelled_by) REFERENCES profiles(id);

ALTER TABLE public.invoice_cancellations ADD CONSTRAINT invoice_cancellations_invoice_id_fkey FOREIGN KEY (invoice_id) REFERENCES invoices(id) ON DELETE RESTRICT;

ALTER TABLE public.invoice_cancellations ADD CONSTRAINT invoice_cancellations_original_client_id_fkey FOREIGN KEY (original_client_id) REFERENCES clients(id);

ALTER TABLE public.invoice_closures ADD CONSTRAINT fk_invoice_closures_closure_id FOREIGN KEY (closure_id) REFERENCES service_closures(id) ON DELETE CASCADE;

ALTER TABLE public.invoice_closures ADD CONSTRAINT fk_invoice_closures_invoice_id FOREIGN KEY (invoice_id) REFERENCES invoices(id) ON DELETE CASCADE;

ALTER TABLE public.invoice_closures ADD CONSTRAINT invoice_closures_closure_id_fkey FOREIGN KEY (closure_id) REFERENCES service_closures(id) ON DELETE CASCADE;

ALTER TABLE public.invoice_closures ADD CONSTRAINT invoice_closures_invoice_id_fkey FOREIGN KEY (invoice_id) REFERENCES invoices(id) ON DELETE CASCADE;
...
ALTER TABLE public.invoice_services ADD CONSTRAINT invoice_services_service_id_fkey FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE CASCADE;

ALTER TABLE public.invoices ADD CONSTRAINT invoices_client_id_fkey FOREIGN KEY (client_id) REFERENCES clients(id);

ALTER TABLE public.invoices ADD CONSTRAINT invoices_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.invoices ADD CONSTRAINT invoices_payment_term_id_fkey FOREIGN KEY (payment_term_id) REFERENCES payment_terms(id);

ALTER TABLE public.notification_logs ADD CONSTRAINT notification_logs_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;

ALTER TABLE public.notification_settings ADD CONSTRAINT notification_settings_user_id_fkey FOREIGN KEY (user_id) REFERENCES profiles(id) ON DELETE CASCADE;

ALTER TABLE public.notifications ADD CONSTRAINT notifications_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;

ALTER TABLE public.operator_documents ADD CONSTRAINT operator_documents_operator_id_fkey FOREIGN KEY (operator_id) REFERENCES operators(id) ON DELETE CASCADE;

ALTER TABLE public.operator_documents ADD CONSTRAINT operator_documents_uploaded_by_fkey FOREIGN KEY (uploaded_by) REFERENCES profiles(id);

ALTER TABLE public.operators ADD CONSTRAINT fk_operators_user_id FOREIGN KEY (user_id) REFERENCES profiles(id) ON DELETE SET NULL;

ALTER TABLE public.operators ADD CONSTRAINT operators_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.patent_search_history ADD CONSTRAINT patent_search_history_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id);

ALTER TABLE public.payment_applications ADD CONSTRAINT payment_applications_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.payment_applications ADD CONSTRAINT payment_applications_invoice_id_fkey FOREIGN KEY (invoice_id) REFERENCES invoices(id) ON DELETE CASCADE;
...
ALTER TABLE public.payment_terms ADD CONSTRAINT payment_terms_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);
...
ALTER TABLE public.purchase_voids ADD CONSTRAINT purchase_voids_replacement_supplier_id_fkey FOREIGN KEY (replacement_supplier_id) REFERENCES suppliers(id);

ALTER TABLE public.purchase_voids ADD CONSTRAINT purchase_voids_voided_by_fkey FOREIGN KEY (voided_by) REFERENCES auth.users(id);
...
ALTER TABLE public.quick_entries ADD CONSTRAINT quick_entries_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.route_tolls ADD CONSTRAINT route_tolls_route_id_fkey FOREIGN KEY (route_id) REFERENCES routes(id) ON DELETE CASCADE;

ALTER TABLE public.route_tolls ADD CONSTRAINT route_tolls_toll_station_id_fkey FOREIGN KEY (toll_station_id) REFERENCES toll_stations(id) ON DELETE CASCADE;

ALTER TABLE public.routes ADD CONSTRAINT routes_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.saved_locations ADD CONSTRAINT saved_locations_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id) ON DELETE SET NULL;

ALTER TABLE public.scheduled_payments ADD CONSTRAINT scheduled_payments_supplier_invoice_id_fkey FOREIGN KEY (supplier_invoice_id) REFERENCES supplier_invoices(id) ON DELETE CASCADE;

ALTER TABLE public.service_cash_receipts ADD CONSTRAINT service_cash_receipts_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.service_cash_receipts ADD CONSTRAINT service_cash_receipts_service_id_fkey FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE CASCADE;

ALTER TABLE public.service_change_history ADD CONSTRAINT service_change_history_changed_by_fkey FOREIGN KEY (changed_by) REFERENCES profiles(id);

ALTER TABLE public.service_change_history ADD CONSTRAINT service_change_history_service_id_fkey FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE CASCADE;

ALTER TABLE public.service_closures ADD CONSTRAINT service_closures_client_id_fkey FOREIGN KEY (client_id) REFERENCES clients(id);

ALTER TABLE public.service_closures ADD CONSTRAINT service_closures_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.service_costs ADD CONSTRAINT service_costs_crane_id_fkey FOREIGN KEY (crane_id) REFERENCES cranes(id) ON DELETE SET NULL;

ALTER TABLE public.service_costs ADD CONSTRAINT service_costs_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.service_costs ADD CONSTRAINT service_costs_operator_id_fkey FOREIGN KEY (operator_id) REFERENCES operators(id) ON DELETE SET NULL;

ALTER TABLE public.service_costs ADD CONSTRAINT service_costs_service_id_fkey FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE CASCADE;

ALTER TABLE public.service_rates ADD CONSTRAINT service_rates_client_id_fkey FOREIGN KEY (client_id) REFERENCES clients(id) ON DELETE CASCADE;

ALTER TABLE public.service_rates ADD CONSTRAINT service_rates_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id) ON DELETE SET NULL;

ALTER TABLE public.service_rates ADD CONSTRAINT service_rates_service_type_id_fkey FOREIGN KEY (service_type_id) REFERENCES service_types(id) ON DELETE SET NULL;

ALTER TABLE public.service_resources ADD CONSTRAINT service_resources_crane_id_fkey FOREIGN KEY (crane_id) REFERENCES cranes(id) ON DELETE SET NULL;

ALTER TABLE public.service_resources ADD CONSTRAINT service_resources_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.service_resources ADD CONSTRAINT service_resources_operator_id_fkey FOREIGN KEY (operator_id) REFERENCES operators(id) ON DELETE SET NULL;

ALTER TABLE public.service_resources ADD CONSTRAINT service_resources_service_id_fkey FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE CASCADE;

ALTER TABLE public.service_types ADD CONSTRAINT service_types_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.service_update_error_logs ADD CONSTRAINT service_update_error_logs_service_id_fkey FOREIGN KEY (service_id) REFERENCES services(id);

ALTER TABLE public.service_update_error_logs ADD CONSTRAINT service_update_error_logs_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id);

ALTER TABLE public.services ADD CONSTRAINT services_client_id_fkey FOREIGN KEY (client_id) REFERENCES clients(id);

ALTER TABLE public.services ADD CONSTRAINT services_crane_id_fkey FOREIGN KEY (crane_id) REFERENCES cranes(id);

ALTER TABLE public.services ADD CONSTRAINT services_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.services ADD CONSTRAINT services_operator_id_fkey FOREIGN KEY (operator_id) REFERENCES operators(id);

ALTER TABLE public.services ADD CONSTRAINT services_outsourced_provider_id_fkey FOREIGN KEY (outsourced_provider_id) REFERENCES inventory_suppliers(id) ON DELETE SET NULL;

ALTER TABLE public.services ADD CONSTRAINT services_related_service_id_fkey FOREIGN KEY (related_service_id) REFERENCES services(id) ON DELETE SET NULL;

ALTER TABLE public.services ADD CONSTRAINT services_service_type_id_fkey FOREIGN KEY (service_type_id) REFERENCES service_types(id);

ALTER TABLE public.services ADD CONSTRAINT services_third_party_client_id_fkey FOREIGN KEY (third_party_client_id) REFERENCES clients(id);

ALTER TABLE public.supplier_categories ADD CONSTRAINT supplier_categories_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.supplier_invoice_items ADD CONSTRAINT supplier_invoice_items_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);
...
ALTER TABLE public.supplier_invoice_items ADD CONSTRAINT supplier_invoice_items_movement_id_fkey FOREIGN KEY (movement_id) REFERENCES inventory_movements(id) ON DELETE SET NULL;
...
ALTER TABLE public.supplier_invoices ADD CONSTRAINT supplier_invoices_supplier_id_fkey FOREIGN KEY (supplier_id) REFERENCES inventory_suppliers(id) ON DELETE CASCADE;

ALTER TABLE public.supplier_payments ADD CONSTRAINT supplier_payments_cost_id_fkey FOREIGN KEY (cost_id) REFERENCES costs(id);

ALTER TABLE public.supplier_payments ADD CONSTRAINT supplier_payments_crane_id_fkey FOREIGN KEY (crane_id) REFERENCES cranes(id);

ALTER TABLE public.supplier_payments ADD CONSTRAINT supplier_payments_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.supplier_payments ADD CONSTRAINT supplier_payments_supplier_id_fkey FOREIGN KEY (supplier_id) REFERENCES inventory_suppliers(id);

ALTER TABLE public.supplier_payments ADD CONSTRAINT supplier_payments_supplier_invoice_id_fkey FOREIGN KEY (supplier_invoice_id) REFERENCES supplier_invoices(id) ON DELETE SET NULL;

ALTER TABLE public.supplier_payments ADD CONSTRAINT supplier_payments_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id);

ALTER TABLE public.suppliers ADD CONSTRAINT suppliers_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.suppliers ADD CONSTRAINT suppliers_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id);

ALTER TABLE public.toll_rates ADD CONSTRAINT toll_rates_toll_station_id_fkey FOREIGN KEY (toll_station_id) REFERENCES toll_stations(id) ON DELETE CASCADE;

ALTER TABLE public.trip_estimates ADD CONSTRAINT trip_estimates_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);

ALTER TABLE public.trip_estimates ADD CONSTRAINT trip_estimates_service_id_fkey FOREIGN KEY (service_id) REFERENCES services(id);

ALTER TABLE public.user_invitations ADD CONSTRAINT user_invitations_user_id_fkey FOREIGN KEY (user_id) REFERENCES profiles(id) ON DELETE CASCADE;

ALTER TABLE public.user_module_permissions ADD CONSTRAINT user_module_permissions_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id);
...
ALTER TABLE public.vehicle_brands ADD CONSTRAINT vehicle_brands_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);

ALTER TABLE public.vehicle_models ADD CONSTRAINT vehicle_models_brand_id_fkey FOREIGN KEY (brand_id) REFERENCES vehicle_brands(id) ON DELETE CASCADE;

ALTER TABLE public.vehicle_models ADD CONSTRAINT vehicle_models_created_by_fkey FOREIGN KEY (created_by) REFERENCES profiles(id);
...
CREATE INDEX idx_invoices_overdue_check ON public.invoices USING btree (status, due_date) WHERE (status = ANY (ARRAY['sent'::invoice_status, 'draft'::invoice_status, 'overdue'::invoice_status]));
...
-- ===== FUNCIONES =====
CREATE OR REPLACE FUNCTION public.admin_create_user(p_email text, p_full_name text, p_role app_role, p_client_id uuid DEFAULT NULL::uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.apply_payment_selective(p_payment_id uuid, p_fiscal_numbers text[], p_apply_only_to_specified boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.assign_user_client(target_user_id uuid, target_client_id uuid DEFAULT NULL::uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.cascade_delete_cost(p_cost_id uuid)
 RETURNS TABLE(deleted_costs integer, deleted_crane_parts integer, deleted_inventory_movements integer, deleted_supplier_payments integer, deleted_supplier_invoices integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.create_automatic_payment_for_invoice(p_invoice_id uuid, p_payment_date date DEFAULT CURRENT_DATE)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.create_inventory_consumption_movement(p_inventory_item_id uuid, p_quantity integer, p_crane_id uuid, p_operator_id uuid DEFAULT NULL::uuid, p_reference_document text DEFAULT NULL::text, p_observations text DEFAULT NULL::text, p_unit_cost numeric DEFAULT NULL::numeric)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.find_matching_costs_for_invoice(p_supplier_rut text, p_amount numeric, p_date_from date, p_date_to date)
 RETURNS TABLE(id uuid, description text, amount numeric, date date, payment_date date, supplier_name text, supplier_payment_id uuid, has_invoice boolean)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.fix_existing_invoice_inconsistencies()
 RETURNS TABLE(invoice_id uuid, old_paid_amount numeric, new_paid_amount numeric, old_status invoice_status, new_status invoice_status)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_default_cost_category_id()
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.get_or_create_inventory_supplier(p_name text, p_rut text, p_email text, p_phone text, p_address text, p_contact_person text, p_category text, p_subcategory text, p_notes text, p_is_active boolean)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.import_xml_batch(p_payload jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.is_admin_user()
 ...
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$ SELECT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND role = 'admin'); $function$
;
...
CREATE OR REPLACE FUNCTION public.list_operators_config()
 RETURNS TABLE(operator_id uuid, operator_name text, email text, user_id_set boolean, has_op_role boolean, services_direct bigint, services_resource bigint)
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.log_import_batch_record()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.mark_supplier_payment_as_paid(p_payment_id uuid, p_paid_date date DEFAULT CURRENT_DATE)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.sync_existing_supplier_payments_to_costs()
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.sync_specific_income_to_payment(p_income_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.update_payment_applied_amount()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO 'public', 'pg_temp'
AS $function$
...
CREATE OR REPLACE FUNCTION public.validate_payment_application_amount(p_invoice_id uuid, p_new_amount numeric, p_excluding_application_id uuid DEFAULT NULL::uuid)
 RETURNS boolean
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.void_inventory_purchase(p_cost_id uuid, p_reason text, p_replacement_supplier_id uuid DEFAULT NULL::uuid, p_revert_payment boolean DEFAULT true, p_revert_invoice boolean DEFAULT true)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.check_cost_duplicates(p_date date, p_amount numeric, p_description text, p_folio text DEFAULT NULL::text, p_tolerance_percent numeric DEFAULT 5)
 RETURNS TABLE(id uuid, date date, description text, amount numeric, service_folio text, created_at timestamp with time zone, match_type text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
AS $function$
...
CREATE OR REPLACE FUNCTION public.check_operator_not_excluded()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO 'public', 'pg_temp'
AS $function$
...
CREATE OR REPLACE FUNCTION public.fix_specific_payment_issue(p_payment_id uuid DEFAULT '019354ad-1fc6-7a97-9faa-f48fa2cb6abc'::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.get_document_expiry_alerts()
 RETURNS TABLE(crane_id uuid, crane_license_plate text, document_type text, expiry_date date, days_until_expiry integer)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_maintenance_with_cost(maintenance_id_param uuid)
 RETURNS TABLE(maintenance_id uuid, maintenance_description text, maintenance_cost numeric, maintenance_status text, cost_id uuid, cost_amount numeric, cost_description text, cost_date date)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.import_xml_costs(p_payload jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.merge_inventory_items(p_master_item_id uuid, p_duplicate_item_ids uuid[], p_master_name text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.migrate_legacy_crane_parts_data(p_crane_id uuid DEFAULT NULL::uuid)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.simple_payment_cleanup()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.sync_crane_part_to_inventory(p_part_name text, p_inventory_item_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.sync_inventory_consumption_to_parts()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.update_supplier_payment_cost_links_updated_at()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.validate_service_update_data(p_service_id uuid, p_service_data jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.approve_pending_user(target_user_id uuid, new_role app_role DEFAULT 'viewer'::app_role, target_client_id uuid DEFAULT NULL::uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.check_for_duplicate_payment(p_client_id uuid, p_amount numeric, p_payment_date date, p_tolerance_days integer DEFAULT 3)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.check_supplier_duplicates(p_rut text, p_name text DEFAULT NULL::text)
 RETURNS TABLE(id uuid, name text, rut text, email text, phone text, is_active boolean, created_at timestamp with time zone, match_type text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
AS $function$
...
CREATE OR REPLACE FUNCTION public.create_cost_with_payment_link(p_payment_id uuid, p_supplier_name text, p_amount numeric, p_description text, p_category text, p_paid_date date, p_cost_category_mapping jsonb DEFAULT '{}'::jsonb, p_default_category text DEFAULT 'Gastos de Proveedores'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.debug_service_states()
 RETURNS TABLE(service_folio text, current_status service_status, invoice_folio text, should_be_invoiced boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.fix_existing_payment_inconsistencies()
 RETURNS TABLE(payment_id uuid, old_applied_amount numeric, new_applied_amount numeric, old_status payment_status, new_status payment_status)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.generate_service_cash_receipt_folio()
 RETURNS text
 LANGUAGE sql
 STABLE
 SET search_path TO 'public', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.get_notification_summary()
 RETURNS TABLE(total_count bigint, unread_count bigint, critical_count bigint, categories jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_parts_traceability(p_crane_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(part_id uuid, part_name text, supplier text, purchase_date date, purchase_cost numeric, inventory_item_id uuid, inventory_item_name text, current_stock bigint, total_purchased bigint, total_consumed bigint, crane_license_plate text)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_supplier_payment_stats(p_supplier_id uuid)
 RETURNS TABLE(total_pending numeric, total_paid numeric, total_overdue numeric, count_pending integer, count_paid integer, count_overdue integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.import_xml_supplier_documents(p_payload jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.is_operator_user()
 ...
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$ SELECT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND role IN ('admin', 'operator')); $function$
;

CREATE OR REPLACE FUNCTION public.log_audit_entry(p_table_name text, p_operation text, p_old_data jsonb DEFAULT NULL::jsonb, p_new_data jsonb DEFAULT NULL::jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.log_service_update_error(p_service_id uuid, p_error_code text, p_error_message text, p_error_details jsonb DEFAULT NULL::jsonb, p_attempted_data jsonb DEFAULT NULL::jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.mark_costs_paid_batch(p_cost_ids uuid[], p_payment_date date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.merge_suppliers(p_keep_id uuid, p_remove_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.repair_payment_application(p_payment_id uuid, p_invoice_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.safe_update_service(service_id_param uuid, update_data jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.update_commission_payment_date(p_commission_ids uuid[], p_payment_date date DEFAULT NULL::date, p_payment_batch_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.update_service_comprehensive(p_service_id uuid, p_service_data jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.apply_payment_selective(p_payment_id uuid, p_fiscal_numbers text[] DEFAULT NULL::text[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.build_import_batch_summary(p_batch_id uuid)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.check_supplier_invoice_duplicates(p_folio text, p_supplier_rut text DEFAULT NULL::text, p_amount numeric DEFAULT NULL::numeric, p_tolerance_percent numeric DEFAULT 5)
 RETURNS TABLE(id uuid, reference_number text, supplier_id uuid, supplier_name text, supplier_rut text, amount numeric, due_date date, created_at timestamp with time zone, match_type text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
AS $function$
...
CREATE OR REPLACE FUNCTION public.cleanup_orphaned_supplier_costs()
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.create_payment_from_existing_income(p_income_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.detect_duplicate_crane_parts(p_crane_id uuid DEFAULT NULL::uuid)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.diagnose_service_update_issues(service_id_param uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.find_duplicate_suppliers()
 RETURNS TABLE(proveedor_1 text, proveedor_2 text, id_1 uuid, id_2 uuid, rut_1 text, rut_2 text, similitud double precision)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.fix_duplicate_paid_amounts()
 RETURNS TABLE(invoice_id uuid, folio text, old_paid_amount numeric, correct_paid_amount numeric, difference numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.force_update_service_to_invoiced(p_service_id uuid, p_invoice_folio text, p_numero_fiscal text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.get_supplier_sync_stats()
 RETURNS TABLE(supplier_name text, total_payments bigint, total_costs bigint, total_movements bigint, total_amount_paid numeric, total_amount_costs numeric, total_amount_inventory numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
...
CREATE OR REPLACE FUNCTION public.insert_notification_if_not_exists(p_user_id uuid, p_type text, p_title text, p_body text, p_data jsonb DEFAULT NULL::jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.is_operator_user_safe()
 ...
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$ SELECT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND role = 'operator'); $function$
;

CREATE OR REPLACE FUNCTION public.log_cost_snapshot_entry(p_cost_id uuid, p_field_name text, p_old_value text DEFAULT NULL::text, p_new_value text DEFAULT NULL::text, p_change_summary text DEFAULT NULL::text, p_change_context text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.propagate_invoice_folio_to_closure_services()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.search_voidable_inventory_purchases(p_search text DEFAULT NULL::text)
...
CREATE OR REPLACE FUNCTION public.smart_apply_payment(p_payment_id uuid, p_auto_apply boolean DEFAULT true)
 RETURNS TABLE(success boolean, applications_made integer, remaining_amount numeric, message text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.sync_existing_services_to_resources()
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.sync_supplier_payment_cost_deletion()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.update_payment_amounts()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.update_services_to_invoiced_batch(p_service_ids uuid[], p_invoice_folio text, p_numero_fiscal text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE VIEW public.orphan_crane_parts_candidates WITH (security_invoker=on) AS
 SELECT p.id AS part_id,
    ...
    c.date AS cost_date
   FROM crane_parts p
     LEFT JOIN supplier_payments sp ON sp.paid_date >= (p.created_at::date - '7 days'::interval) AND sp.paid_date <= (p.created_at::date + '7 days'::interval)
     LEFT JOIN costs c ON c.date >= (p.created_at::date - '7 days'::interval) AND c.date <= (p.created_at::date + '7 days'::interval)
...
CREATE OR REPLACE VIEW public.services_with_excess_summary WITH (security_invoker=on) AS
 SELECT s.id,
   ...
   FROM services s
     ...
     LEFT JOIN services rs ON s.related_service_id = rs.id
     LEFT JOIN clients rc ON rs.client_id = rc.id
  WHERE s.has_excess = true OR (s.service_relationship_type = ANY (ARRAY['main'::text, 'excess'::text]));

-- ===== TRIGGERS =====
CREATE TRIGGER trg_backup_email_config_updated_at BEFORE UPDATE ON public.backup_email_config FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_company_data_updated_at BEFORE UPDATE ON public.company_data FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER handle_updated_at_cost_centers BEFORE UPDATE ON public.cost_centers FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_cost_subcategories_updated_at BEFORE UPDATE ON public.cost_subcategories FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER create_supplier_payment_from_cost_trigger AFTER INSERT OR UPDATE ON public.costs FOR EACH ROW WHEN ((new.supplier_id IS NOT NULL)) EXECUTE FUNCTION create_supplier_payment_from_cost();

CREATE TRIGGER enforce_commission_batch_consistency_trigger BEFORE INSERT OR UPDATE OF payment_batch_id, payment_date, operator_id, category_id ON public.costs FOR EACH ROW EXECUTE FUNCTION enforce_commission_batch_consistency();

CREATE TRIGGER handle_immediate_consumption_update_trigger AFTER UPDATE ON public.costs FOR EACH ROW EXECUTE FUNCTION handle_immediate_consumption_update();

CREATE TRIGGER handle_updated_at_costs BEFORE UPDATE ON public.costs FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER log_import_batch_record_costs AFTER INSERT ON public.costs FOR EACH ROW EXECUTE FUNCTION log_import_batch_record();

CREATE TRIGGER prevent_duplicate_commissions_trigger BEFORE INSERT OR UPDATE ON public.costs FOR EACH ROW EXECUTE FUNCTION prevent_duplicate_commissions();

CREATE TRIGGER prevent_excluded_operator_commissions BEFORE INSERT ON public.costs FOR EACH ROW EXECUTE FUNCTION check_operator_not_excluded();

CREATE TRIGGER prevent_maintenance_cost_duplicates BEFORE INSERT OR UPDATE ON public.costs FOR EACH ROW EXECUTE FUNCTION prevent_duplicate_maintenance_costs();

CREATE TRIGGER prevent_non_admin_updates_on_paid_costs_trigger BEFORE UPDATE ON public.costs FOR EACH ROW EXECUTE FUNCTION prevent_non_admin_updates_on_paid_costs();

CREATE TRIGGER sync_cost_deletion_cascade_trigger BEFORE DELETE ON public.costs FOR EACH ROW EXECUTE FUNCTION sync_cost_deletion_cascade();

CREATE TRIGGER sync_inventory_cost_trigger AFTER INSERT ON public.costs FOR EACH ROW EXECUTE FUNCTION sync_inventory_cost_to_movement();

CREATE TRIGGER trg_audit_costs AFTER INSERT OR DELETE OR UPDATE ON public.costs FOR EACH ROW EXECUTE FUNCTION log_audit_changes();

CREATE TRIGGER trigger_assign_cost_center BEFORE INSERT ON public.costs FOR EACH ROW EXECUTE FUNCTION assign_default_cost_center();

CREATE TRIGGER trigger_assign_cost_center_on_update BEFORE UPDATE ON public.costs FOR EACH ROW EXECUTE FUNCTION assign_default_cost_center();

CREATE TRIGGER trigger_ensure_commission_operator_id BEFORE INSERT OR UPDATE ON public.costs FOR EACH ROW EXECUTE FUNCTION ensure_commission_operator_id();

CREATE TRIGGER trigger_track_cost_changes AFTER INSERT OR DELETE OR UPDATE ON public.costs FOR EACH ROW EXECUTE FUNCTION track_cost_changes();

CREATE TRIGGER update_crane_consumption_rates_updated_at BEFORE UPDATE ON public.crane_consumption_rates FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_crane_documents_updated_at BEFORE UPDATE ON public.crane_documents FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_crane_expiry_trigger AFTER INSERT OR UPDATE ON public.crane_documents FOR EACH ROW EXECUTE FUNCTION update_crane_expiry_on_document_upload();

CREATE TRIGGER auto_maintenance_status_trigger BEFORE UPDATE ON public.crane_maintenance FOR EACH ROW EXECUTE FUNCTION auto_update_maintenance_status();

CREATE TRIGGER create_cost_from_maintenance_trigger AFTER UPDATE ON public.crane_maintenance FOR EACH ROW EXECUTE FUNCTION create_cost_from_maintenance();

CREATE TRIGGER update_crane_maintenance_updated_at BEFORE UPDATE ON public.crane_maintenance FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER crane_parts_delete_cost BEFORE DELETE ON public.crane_parts FOR EACH ROW EXECUTE FUNCTION delete_cost_for_crane_part();

CREATE TRIGGER crane_parts_update_cost AFTER UPDATE ON public.crane_parts FOR EACH ROW EXECUTE FUNCTION update_cost_for_crane_part();

CREATE TRIGGER sync_parts_purchase_to_inventory_trigger AFTER INSERT ON public.crane_parts FOR EACH ROW EXECUTE FUNCTION sync_parts_purchase_to_inventory();

CREATE TRIGGER trigger_calculate_crane_part_total_value BEFORE INSERT OR UPDATE ON public.crane_parts FOR EACH ROW EXECUTE FUNCTION calculate_crane_part_total_value();

CREATE TRIGGER trigger_track_crane_part_changes AFTER INSERT OR DELETE OR UPDATE ON public.crane_parts FOR EACH ROW EXECUTE FUNCTION track_crane_part_changes();

CREATE TRIGGER update_crane_parts_updated_at BEFORE UPDATE ON public.crane_parts FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trg_cranes_propagate_company AFTER UPDATE OF owner_company_rut, owner_company_name ON public.cranes FOR EACH ROW EXECUTE FUNCTION sync_services_on_crane_company_change();

CREATE TRIGGER update_cranes_updated_at BEFORE UPDATE ON public.cranes FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_document_alerts_updated_at BEFORE UPDATE ON public.document_alerts FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER set_import_rut_mappings_updated_at BEFORE UPDATE ON public.import_rut_mappings FOR EACH ROW EXECUTE FUNCTION set_import_rut_mappings_updated_at();
...
CREATE TRIGGER update_income_subcategories_updated_at BEFORE UPDATE ON public.income_subcategories FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_incomes_updated_at BEFORE UPDATE ON public.incomes FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_inventory_alerts_updated_at BEFORE UPDATE ON public.inventory_alerts FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER log_import_batch_record_inventory_movements AFTER INSERT ON public.inventory_movements FOR EACH ROW EXECUTE FUNCTION log_import_batch_record();

CREATE TRIGGER sync_inventory_exit_to_crane_parts_trigger AFTER INSERT OR UPDATE ON public.inventory_movements FOR EACH ROW EXECUTE FUNCTION sync_inventory_exit_to_crane_parts();

CREATE TRIGGER sync_inventory_to_cost_trigger AFTER INSERT ON public.inventory_movements FOR EACH ROW EXECUTE FUNCTION sync_inventory_to_supplier_and_cost();

CREATE TRIGGER trg_audit_inventory_movements AFTER INSERT ON public.inventory_movements FOR EACH ROW EXECUTE FUNCTION log_audit_changes();

CREATE TRIGGER trg_normalize_inventory_movement_timestamp BEFORE INSERT ON public.inventory_movements FOR EACH ROW EXECUTE FUNCTION normalize_inventory_movement_timestamp();

CREATE TRIGGER trigger_fill_exit_costs BEFORE INSERT OR UPDATE ON public.inventory_movements FOR EACH ROW EXECUTE FUNCTION fill_exit_costs();

CREATE TRIGGER trigger_track_inventory_movement_changes AFTER INSERT OR DELETE OR UPDATE ON public.inventory_movements FOR EACH ROW EXECUTE FUNCTION track_inventory_movement_changes();

CREATE TRIGGER trigger_update_inventory_stock AFTER INSERT OR DELETE OR UPDATE ON public.inventory_movements FOR EACH ROW EXECUTE FUNCTION update_inventory_stock();

CREATE TRIGGER trigger_check_inventory_alerts AFTER UPDATE OF current_quantity ON public.inventory_stock FOR EACH ROW EXECUTE FUNCTION check_inventory_alerts();
...
CREATE TRIGGER log_import_batch_record_inventory_suppliers AFTER INSERT ON public.inventory_suppliers FOR EACH ROW EXECUTE FUNCTION log_import_batch_record();

CREATE TRIGGER update_inventory_suppliers_updated_at BEFORE UPDATE ON public.inventory_suppliers FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER propagate_invoice_folio_trigger AFTER INSERT ON public.invoice_closures FOR EACH ROW EXECUTE FUNCTION propagate_invoice_folio_to_closure_services();

CREATE TRIGGER simple_invoice_closures_updated_at BEFORE UPDATE ON public.invoice_closures FOR EACH ROW EXECUTE FUNCTION simple_update_timestamp();

CREATE TRIGGER invoices_enforce_product_service_description_trg BEFORE INSERT OR UPDATE ON public.invoices FOR EACH ROW EXECUTE FUNCTION enforce_product_service_description();

CREATE TRIGGER simple_invoices_updated_at BEFORE UPDATE ON public.invoices FOR EACH ROW EXECUTE FUNCTION simple_update_timestamp();
...
CREATE TRIGGER update_operator_documents_updated_at BEFORE UPDATE ON public.operator_documents FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER maintain_payment_consistency_trigger AFTER INSERT OR DELETE OR UPDATE ON public.payment_applications FOR EACH ROW EXECUTE FUNCTION maintain_payment_consistency();

CREATE TRIGGER payment_applications_auto_update_invoice_status AFTER INSERT OR DELETE OR UPDATE ON public.payment_applications FOR EACH ROW EXECUTE FUNCTION update_invoice_status_from_payments();

CREATE TRIGGER prevent_invoice_overpayment_trigger BEFORE INSERT OR UPDATE ON public.payment_applications FOR EACH ROW EXECUTE FUNCTION prevent_invoice_overpayment();

CREATE TRIGGER update_invoice_amounts_trigger AFTER INSERT OR DELETE OR UPDATE ON public.payment_applications FOR EACH ROW EXECUTE FUNCTION update_invoice_amounts();

CREATE TRIGGER update_payment_amounts_trigger AFTER INSERT OR DELETE OR UPDATE ON public.payment_applications FOR EACH ROW EXECUTE FUNCTION update_payment_amounts();

CREATE TRIGGER prevent_duplicate_payments_trigger BEFORE INSERT OR UPDATE ON public.payments FOR EACH ROW EXECUTE FUNCTION prevent_duplicate_payments();

CREATE TRIGGER prevent_payment_duplicates BEFORE INSERT ON public.payments FOR EACH ROW EXECUTE FUNCTION prevent_duplicate_payments();

CREATE TRIGGER update_payment_amounts BEFORE INSERT OR UPDATE ON public.payments FOR EACH ROW EXECUTE FUNCTION update_payment_remaining_amount();

CREATE TRIGGER update_payments_updated_at BEFORE UPDATE ON public.payments FOR EACH ROW EXECUTE FUNCTION simple_update_timestamp();

CREATE TRIGGER on_user_registration_update_invitation AFTER INSERT ON public.profiles FOR EACH ROW EXECUTE FUNCTION handle_user_invitation_acceptance();
...
CREATE TRIGGER update_quick_entries_updated_at BEFORE UPDATE ON public.quick_entries FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_scheduled_payments_updated_at BEFORE UPDATE ON public.scheduled_payments FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_service_closures_updated_at BEFORE UPDATE ON public.service_closures FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER prevent_duplicate_service_commissions_trigger BEFORE INSERT OR UPDATE ON public.service_costs FOR EACH ROW EXECUTE FUNCTION prevent_duplicate_service_commissions();

CREATE TRIGGER update_service_costs_updated_at BEFORE UPDATE ON public.service_costs FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_service_rates_updated_at BEFORE UPDATE ON public.service_rates FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER cascade_delete_service_trigger BEFORE DELETE ON public.services FOR EACH ROW EXECUTE FUNCTION cascade_delete_service_data();

CREATE TRIGGER generate_commission_on_service_completion_trigger AFTER UPDATE ON public.services FOR EACH ROW EXECUTE FUNCTION generate_commission_on_service_completion();

CREATE TRIGGER prevent_excess_of_excess_trigger BEFORE INSERT OR UPDATE ON public.services FOR EACH ROW EXECUTE FUNCTION prevent_excess_of_excess();

CREATE TRIGGER trg_services_sync_company_ins BEFORE INSERT ON public.services FOR EACH ROW EXECUTE FUNCTION sync_service_company_from_crane();

CREATE TRIGGER trg_services_sync_company_upd BEFORE UPDATE OF crane_id ON public.services FOR EACH ROW EXECUTE FUNCTION sync_service_company_from_crane();

CREATE TRIGGER trigger_auto_update_service_invoice_status BEFORE UPDATE ON public.services FOR EACH ROW EXECUTE FUNCTION auto_update_service_invoice_status();

CREATE TRIGGER trigger_delete_commissions_on_service_delete BEFORE DELETE ON public.services FOR EACH ROW EXECUTE FUNCTION delete_commissions_on_service_delete();

CREATE TRIGGER trigger_generate_commission_on_service_completion AFTER UPDATE ON public.services FOR EACH ROW EXECUTE FUNCTION generate_commission_on_service_completion();

CREATE TRIGGER trigger_track_service_changes AFTER INSERT OR DELETE OR UPDATE ON public.services FOR EACH ROW EXECUTE FUNCTION track_service_changes();

CREATE TRIGGER trigger_validate_service_invoice_consistency BEFORE INSERT OR UPDATE ON public.services FOR EACH ROW EXECUTE FUNCTION validate_service_invoice_consistency();

CREATE TRIGGER update_services_updated_at BEFORE UPDATE ON public.services FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trigger_update_supplier_categories_updated_at BEFORE UPDATE ON public.supplier_categories FOR EACH ROW EXECUTE FUNCTION update_supplier_categories_updated_at();

CREATE TRIGGER supplier_invoice_items_touch_updated_at BEFORE UPDATE ON public.supplier_invoice_items FOR EACH ROW EXECUTE FUNCTION touch_supplier_invoice_items_updated_at();

CREATE TRIGGER log_import_batch_record_supplier_invoices AFTER INSERT ON public.supplier_invoices FOR EACH ROW EXECUTE FUNCTION log_import_batch_record();

CREATE TRIGGER supplier_invoices_enforce_product_service_description_trg BEFORE INSERT OR UPDATE ON public.supplier_invoices FOR EACH ROW EXECUTE FUNCTION enforce_product_service_description();

CREATE TRIGGER sync_supplier_invoice_delete_trigger BEFORE DELETE ON public.supplier_invoices FOR EACH ROW EXECUTE FUNCTION sync_supplier_invoice_delete();

CREATE TRIGGER sync_supplier_invoice_update_trigger AFTER UPDATE ON public.supplier_invoices FOR EACH ROW EXECUTE FUNCTION sync_supplier_invoice_update();
...
CREATE TRIGGER create_cost_from_supplier_payment AFTER INSERT OR UPDATE OF status ON public.supplier_payments FOR EACH ROW EXECUTE FUNCTION create_cost_from_supplier_payment();

CREATE TRIGGER log_import_batch_record_supplier_payments AFTER INSERT ON public.supplier_payments FOR EACH ROW EXECUTE FUNCTION log_import_batch_record();

CREATE TRIGGER on_supplier_payment_delete_trigger AFTER DELETE ON public.supplier_payments FOR EACH ROW EXECUTE FUNCTION on_supplier_payment_delete();

CREATE TRIGGER sync_cost_supplier_payment_deletion_trigger BEFORE DELETE ON public.supplier_payments FOR EACH ROW WHEN ((old.cost_id IS NOT NULL)) EXECUTE FUNCTION sync_cost_supplier_payment_deletion();

CREATE TRIGGER sync_supplier_payment_update_to_cost_trigger AFTER UPDATE ON public.supplier_payments FOR EACH ROW WHEN ((new.cost_id IS NOT NULL)) EXECUTE FUNCTION sync_supplier_payment_update_to_cost();

CREATE TRIGGER trg_audit_supplier_payments AFTER INSERT OR DELETE OR UPDATE ON public.supplier_payments FOR EACH ROW EXECUTE FUNCTION log_audit_changes();
...
CREATE TRIGGER update_toll_rates_updated_at BEFORE UPDATE ON public.toll_rates FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_toll_stations_updated_at BEFORE UPDATE ON public.toll_stations FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER sync_role_to_profile_trigger AFTER INSERT OR UPDATE ON public.user_roles FOR EACH ROW EXECUTE FUNCTION sync_role_to_profile();

CREATE TRIGGER update_user_settings_updated_at BEFORE UPDATE ON public.user_settings FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_vehicle_brands_updated_at BEFORE UPDATE ON public.vehicle_brands FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_vehicle_models_updated_at BEFORE UPDATE ON public.vehicle_models FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_whatsapp_message_log_updated_at BEFORE UPDATE ON public.whatsapp_message_log FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- ===== POLICIES (public) =====
CREATE POLICY audit_log_admin_select ON public.audit_log AS PERMISSIVE FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY audit_log_no_direct_insert ON public.audit_log AS PERMISSIVE FOR INSERT TO public WITH CHECK (false);

CREATE POLICY audit_log_user_select ON public.audit_log AS PERMISSIVE FOR SELECT TO authenticated USING (((user_id = auth.uid()) OR has_role(auth.uid(), 'admin'::app_role)));

CREATE POLICY "Admins can insert backup email config" ON public.backup_email_config AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY "Admins can update backup email config" ON public.backup_email_config AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY "Admins can view backup email config" ON public.backup_email_config AS PERMISSIVE FOR SELECT TO authenticated USING (is_admin_user_safe());

CREATE POLICY backup_logs_admin_only ON public.backup_logs AS PERMISSIVE FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY calendar_events_select_auth ON public.calendar_events AS PERMISSIVE FOR SELECT TO authenticated USING (is_authenticated_user_safe());

CREATE POLICY calendar_events_write_admin_operator ON public.calendar_events AS PERMISSIVE FOR ALL TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY clients_admin_full_access ON public.clients AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY clients_operator_assigned_only ON public.clients AS PERMISSIVE FOR SELECT TO authenticated USING ((is_operator_user_safe() AND (EXISTS ( SELECT 1
   FROM operators o
...
CREATE POLICY clients_operator_insert_restricted ON public.clients AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_operator_user_safe() AND (created_by = auth.uid())));

CREATE POLICY clients_own_basic_data ON public.clients AS PERMISSIVE FOR SELECT TO authenticated USING ((is_client_user_safe() AND (id = get_user_client_id_safe())));

CREATE POLICY closure_services_delete_admin ON public.closure_services AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY closure_services_insert_admin ON public.closure_services AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY closure_services_select_auth ON public.closure_services AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY closure_services_update_admin ON public.closure_services AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY company_data_admin_only ON public.company_data AS PERMISSIVE FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY company_data_select_authenticated ON public.company_data AS PERMISSIVE FOR SELECT TO authenticated USING ((auth.role() = 'authenticated'::text));

CREATE POLICY "Admins can manage company profiles" ON public.company_profiles AS PERMISSIVE FOR ALL TO public USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY "Authenticated users can view company profiles" ON public.company_profiles AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY cost_bulk_payment_operations_insert_admin_operator ON public.cost_bulk_payment_operations AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_operator_user());

CREATE POLICY cost_bulk_payment_operations_select_own_or_admin ON public.cost_bulk_payment_operations AS PERMISSIVE FOR SELECT TO authenticated USING (((executed_by = auth.uid()) OR is_admin_user()));

CREATE POLICY cost_categories_delete_admin ON public.cost_categories AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY cost_categories_insert_admin ON public.cost_categories AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY cost_categories_select_auth ON public.cost_categories AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY cost_categories_update_admin ON public.cost_categories AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY cost_centers_delete_admin ON public.cost_centers AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY cost_centers_insert_admin ON public.cost_centers AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY cost_centers_select_auth ON public.cost_centers AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY cost_centers_update_admin ON public.cost_centers AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY cch_no_direct_insert ON public.cost_change_history AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (false);

CREATE POLICY cch_select_admin ON public.cost_change_history AS PERMISSIVE FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY cost_inventory_items_delete_admin ON public.cost_inventory_items AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY cost_inventory_items_insert_admin ON public.cost_inventory_items AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY cost_inventory_items_select_auth ON public.cost_inventory_items AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY cost_inventory_items_update_admin ON public.cost_inventory_items AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY cost_subcategories_admin_write ON public.cost_subcategories AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY cost_subcategories_read ON public.cost_subcategories AS PERMISSIVE FOR SELECT TO authenticated USING (is_authenticated_user_safe());

CREATE POLICY costs_admin_full_access ON public.costs AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe());

CREATE POLICY costs_client_own_services ON public.costs AS PERMISSIVE FOR SELECT TO authenticated USING ((is_client_user_safe() AND (service_id IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM services s
  WHERE ((s.id = costs.service_id) AND (s.client_id = get_user_client_id_safe()))))));

CREATE POLICY costs_operator_scoped_access ON public.costs AS PERMISSIVE FOR SELECT TO authenticated USING ((is_operator_user_safe() AND ((EXISTS ( SELECT 1
   FROM operators o
...
CREATE POLICY crane_consumption_rates_delete_admin ON public.crane_consumption_rates AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY crane_consumption_rates_insert_admin ON public.crane_consumption_rates AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY crane_consumption_rates_select_auth ON public.crane_consumption_rates AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY crane_consumption_rates_update_admin ON public.crane_consumption_rates AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY crane_documents_admin_operator_all ON public.crane_documents AS PERMISSIVE FOR ALL TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY crane_maintenance_admin_full ON public.crane_maintenance AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY crane_maintenance_operator_select ON public.crane_maintenance AS PERMISSIVE FOR SELECT TO authenticated USING (is_operator_user_safe());

CREATE POLICY cpch_no_direct_insert ON public.crane_part_change_history AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (false);

CREATE POLICY cpch_select_admin ON public.crane_part_change_history AS PERMISSIVE FOR SELECT TO authenticated USING ((has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'operator'::app_role)));

CREATE POLICY crane_parts_auth_only ON public.crane_parts AS PERMISSIVE FOR ALL TO public USING ((auth.role() = 'authenticated'::text));

CREATE POLICY cranes_select_auth ON public.cranes AS PERMISSIVE FOR SELECT TO authenticated USING (is_authenticated_user_safe());

CREATE POLICY cranes_write_admin ON public.cranes AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY creditors_select_admin_viewer ON public.creditors AS PERMISSIVE FOR SELECT TO authenticated USING ((get_current_user_role_safe() = ANY (ARRAY['admin'::app_role, 'viewer'::app_role])));

CREATE POLICY creditors_write_admin_only ON public.creditors AS PERMISSIVE FOR ALL TO authenticated USING ((get_current_user_role_safe() = 'admin'::app_role)) WITH CHECK ((get_current_user_role_safe() = 'admin'::app_role));

CREATE POLICY debt_installments_select_admin_viewer ON public.debt_installments AS PERMISSIVE FOR SELECT TO authenticated USING ((get_current_user_role_safe() = ANY (ARRAY['admin'::app_role, 'viewer'::app_role])));

CREATE POLICY debt_installments_write_admin_only ON public.debt_installments AS PERMISSIVE FOR ALL TO authenticated USING ((get_current_user_role_safe() = 'admin'::app_role)) WITH CHECK ((get_current_user_role_safe() = 'admin'::app_role));

CREATE POLICY debt_payments_select_admin_viewer ON public.debt_payments AS PERMISSIVE FOR SELECT TO authenticated USING ((get_current_user_role_safe() = ANY (ARRAY['admin'::app_role, 'viewer'::app_role])));

CREATE POLICY debt_payments_write_admin_only ON public.debt_payments AS PERMISSIVE FOR ALL TO authenticated USING ((get_current_user_role_safe() = 'admin'::app_role)) WITH CHECK ((get_current_user_role_safe() = 'admin'::app_role));

CREATE POLICY debts_select_admin_viewer ON public.debts AS PERMISSIVE FOR SELECT TO authenticated USING ((get_current_user_role_safe() = ANY (ARRAY['admin'::app_role, 'viewer'::app_role])));

CREATE POLICY debts_write_admin_only ON public.debts AS PERMISSIVE FOR ALL TO authenticated USING ((get_current_user_role_safe() = 'admin'::app_role)) WITH CHECK ((get_current_user_role_safe() = 'admin'::app_role));

CREATE POLICY document_alerts_delete_admin ON public.document_alerts AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY document_alerts_insert_admin ON public.document_alerts AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY document_alerts_select_auth ON public.document_alerts AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY document_alerts_update_admin ON public.document_alerts AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY allow_insert_anon ON public.frontend_error_logs AS PERMISSIVE FOR INSERT TO anon WITH CHECK (true);

CREATE POLICY allow_insert_authenticated ON public.frontend_error_logs AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (true);

CREATE POLICY allow_select_admin ON public.frontend_error_logs AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'admin'::app_role)))));

CREATE POLICY fuel_prices_delete_admin ON public.fuel_prices AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY fuel_prices_insert_admin ON public.fuel_prices AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY fuel_prices_select_auth ON public.fuel_prices AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY fuel_prices_update_admin ON public.fuel_prices AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY import_batch_records_insert_own ON public.import_batch_records AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((EXISTS ( SELECT 1
   FROM import_batches
  WHERE ((import_batches.id = import_batch_records.batch_id) AND (import_batches.created_by = auth.uid())))));

CREATE POLICY import_batch_records_select_own ON public.import_batch_records AS PERMISSIVE FOR SELECT TO public USING ((batch_id IN ( SELECT import_batches.id
   FROM import_batches
  WHERE (import_batches.created_by = auth.uid()))));

CREATE POLICY import_batches_insert_admin_operator ON public.import_batches AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (((created_by = auth.uid()) AND (is_admin_user_safe() OR is_operator_user_safe())));

CREATE POLICY import_batches_select_own ON public.import_batches AS PERMISSIVE FOR SELECT TO public USING ((created_by = auth.uid()));

CREATE POLICY import_batches_update_own ON public.import_batches AS PERMISSIVE FOR UPDATE TO public USING ((created_by = auth.uid())) WITH CHECK ((created_by = auth.uid()));

CREATE POLICY "Users manage own import logs" ON public.import_history_log AS PERMISSIVE FOR ALL TO public USING ((auth.uid() = organization_id)) WITH CHECK ((auth.uid() = organization_id));

CREATE POLICY "Users manage own mappings" ON public.import_rut_mappings AS PERMISSIVE FOR ALL TO public USING ((auth.uid() = organization_id)) WITH CHECK ((auth.uid() = organization_id));

CREATE POLICY income_categories_admin_all ON public.income_categories AS PERMISSIVE FOR ALL TO public USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY income_categories_select_authenticated ON public.income_categories AS PERMISSIVE FOR SELECT TO public USING (is_authenticated_user_safe());

CREATE POLICY income_subcategories_admin_all ON public.income_subcategories AS PERMISSIVE FOR ALL TO public USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY income_subcategories_select_authenticated ON public.income_subcategories AS PERMISSIVE FOR SELECT TO public USING (is_authenticated_user_safe());

CREATE POLICY incomes_delete_admin ON public.incomes AS PERMISSIVE FOR DELETE TO public USING (is_admin_user_safe());

CREATE POLICY incomes_insert_admin ON public.incomes AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY incomes_select_authenticated ON public.incomes AS PERMISSIVE FOR SELECT TO public USING (is_authenticated_user_safe());

CREATE POLICY incomes_update_admin ON public.incomes AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY inspections_admin_full ON public.inspections AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY inspections_operator_insert ON public.inspections AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_operator_user_safe() AND is_operator_assigned_to_service(service_id) AND (EXISTS ( SELECT 1
   FROM operators o
  WHERE ((o.id = inspections.operator_id) AND (o.user_id = auth.uid()))))));

CREATE POLICY inspections_operator_select ON public.inspections AS PERMISSIVE FOR SELECT TO authenticated USING ((is_operator_user_safe() AND is_operator_assigned_to_service(service_id)));

CREATE POLICY inspections_operator_update ON public.inspections AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_operator_user_safe() AND is_operator_assigned_to_service(service_id))) WITH CHECK ((is_operator_user_safe() AND is_operator_assigned_to_service(service_id)));

CREATE POLICY internal_scheduler_secrets_block_all ON public.internal_scheduler_secrets AS PERMISSIVE FOR ALL TO public USING (false) WITH CHECK (false);

CREATE POLICY inventory_alerts_delete_staff ON public.inventory_alerts AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_alerts_insert_staff ON public.inventory_alerts AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_alerts_select_auth ON public.inventory_alerts AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY inventory_alerts_update_staff ON public.inventory_alerts AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_categories_delete_staff ON public.inventory_categories AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_categories_insert_staff ON public.inventory_categories AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_categories_select_auth ON public.inventory_categories AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY inventory_categories_update_staff ON public.inventory_categories AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_consumptions_delete_staff ON public.inventory_consumptions AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_consumptions_insert_staff ON public.inventory_consumptions AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_consumptions_select_auth ON public.inventory_consumptions AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY inventory_consumptions_update_staff ON public.inventory_consumptions AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_items_delete_staff ON public.inventory_items AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_items_insert_staff ON public.inventory_items AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_items_select_auth ON public.inventory_items AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY inventory_items_update_staff ON public.inventory_items AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_locations_delete_staff ON public.inventory_locations AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_locations_insert_staff ON public.inventory_locations AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_locations_select_auth ON public.inventory_locations AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY inventory_locations_update_staff ON public.inventory_locations AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY imch_no_direct_insert ON public.inventory_movement_change_history AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (false);

CREATE POLICY imch_select_admin ON public.inventory_movement_change_history AS PERMISSIVE FOR SELECT TO authenticated USING ((has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'operator'::app_role)));

CREATE POLICY inventory_movements_delete_staff ON public.inventory_movements AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_movements_insert_staff ON public.inventory_movements AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_movements_select_auth ON public.inventory_movements AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY inventory_movements_update_staff ON public.inventory_movements AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_stock_delete_staff ON public.inventory_stock AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_stock_insert_staff ON public.inventory_stock AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_stock_select_auth ON public.inventory_stock AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY inventory_stock_update_staff ON public.inventory_stock AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY inventory_suppliers_admin_full ON public.inventory_suppliers AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY inventory_suppliers_operator_read ON public.inventory_suppliers AS PERMISSIVE FOR SELECT TO authenticated USING (is_operator_user_safe());

CREATE POLICY "Users can manage their own alert settings" ON public.invoice_alert_settings AS PERMISSIVE FOR ALL TO public USING ((auth.uid() = user_id));

CREATE POLICY invoice_cancellations_admin_full_access ON public.invoice_cancellations AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY invoice_cancellations_scoped_select ON public.invoice_cancellations AS PERMISSIVE FOR SELECT TO authenticated USING ((has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'operator'::app_role) OR (original_client_id = get_user_client_id_safe()) OR (EXISTS ( SELECT 1
   FROM invoices i
  WHERE ((i.id = invoice_cancellations.invoice_id) AND (i.client_id = get_user_client_id_safe()))))));

CREATE POLICY invoice_closures_scoped_select ON public.invoice_closures AS PERMISSIVE FOR SELECT TO authenticated USING ((has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'operator'::app_role) OR (EXISTS ( SELECT 1
   FROM invoices i
  WHERE ((i.id = invoice_closures.invoice_id) AND (i.client_id = get_user_client_id_safe()))))));

CREATE POLICY invoice_closures_write_admin ON public.invoice_closures AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY invoice_services_scoped_select ON public.invoice_services AS PERMISSIVE FOR SELECT TO authenticated USING ((has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'operator'::app_role) OR (EXISTS ( SELECT 1
   FROM invoices i
  WHERE ((i.id = invoice_services.invoice_id) AND (i.client_id = get_user_client_id_safe()))))));

CREATE POLICY invoice_services_write_admin ON public.invoice_services AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY invoices_admin_full_access ON public.invoices AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY invoices_client_own ON public.invoices AS PERMISSIVE FOR SELECT TO authenticated USING ((is_client_user_safe() AND (client_id = get_user_client_id_safe())));

CREATE POLICY invoices_operator_read ON public.invoices AS PERMISSIVE FOR SELECT TO authenticated USING ((is_operator_user_safe() AND (EXISTS ( SELECT 1
   FROM (operators o
...
CREATE POLICY notification_logs_user_access ON public.notification_logs AS PERMISSIVE FOR SELECT TO authenticated USING (((auth.uid() = user_id) AND is_authenticated_user_safe()));

CREATE POLICY notification_settings_own ON public.notification_settings AS PERMISSIVE FOR ALL TO public USING ((auth.uid() = user_id));
...
CREATE POLICY operator_documents_admin_all ON public.operator_documents AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY operator_documents_operator_owner_select ON public.operator_documents AS PERMISSIVE FOR SELECT TO authenticated USING ((is_operator_user_safe() AND (EXISTS ( SELECT 1
   FROM operators o
  WHERE ((o.id = operator_documents.operator_id) AND (o.user_id = auth.uid()))))));

CREATE POLICY operators_admin_full_access ON public.operators AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY operators_admin_operator_read_all ON public.operators AS PERMISSIVE FOR SELECT TO authenticated USING (is_admin_user_safe());

CREATE POLICY operators_read_self ON public.operators AS PERMISSIVE FOR SELECT TO authenticated USING ((is_operator_user_safe() AND (user_id = auth.uid())));

CREATE POLICY patent_search_history_delete_own ON public.patent_search_history AS PERMISSIVE FOR DELETE TO public USING ((auth.uid() = user_id));

CREATE POLICY patent_search_history_insert_own ON public.patent_search_history AS PERMISSIVE FOR INSERT TO public WITH CHECK ((auth.uid() = user_id));

CREATE POLICY patent_search_history_select_own ON public.patent_search_history AS PERMISSIVE FOR SELECT TO public USING ((auth.uid() = user_id));

CREATE POLICY payment_applications_scoped_select ON public.payment_applications AS PERMISSIVE FOR SELECT TO authenticated USING ((has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'operator'::app_role) OR (EXISTS ( SELECT 1
   FROM invoices i
  WHERE ((i.id = payment_applications.invoice_id) AND (i.client_id = get_user_client_id_safe()))))));

CREATE POLICY payment_applications_write_admin ON public.payment_applications AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY payment_terms_delete ON public.payment_terms AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY payment_terms_read ON public.payment_terms AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY payment_terms_update ON public.payment_terms AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY payment_terms_write ON public.payment_terms AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY payments_admin_full_access ON public.payments AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY payments_client_own ON public.payments AS PERMISSIVE FOR SELECT TO authenticated USING ((is_client_user_safe() AND (client_id = get_user_client_id_safe())));

CREATE POLICY payments_operator_read ON public.payments AS PERMISSIVE FOR SELECT TO authenticated USING ((is_operator_user_safe() AND (EXISTS ( SELECT 1
   FROM (operators o
...
CREATE POLICY profiles_insert_own_safe ON public.profiles AS PERMISSIVE FOR INSERT TO public WITH CHECK (((auth.uid() = id) AND (role = 'viewer'::app_role)));
...
CREATE POLICY profiles_update_own_restricted ON public.profiles AS PERMISSIVE FOR UPDATE TO authenticated USING ((auth.uid() = id)) WITH CHECK (((auth.uid() = id) AND (NOT (role IS DISTINCT FROM ( SELECT p.role
   FROM profiles p
  WHERE (p.id = auth.uid())))) AND (NOT (is_active IS DISTINCT FROM ( SELECT p.is_active
   FROM profiles p
  WHERE (p.id = auth.uid()))))));

CREATE POLICY "Admins can insert purchase voids" ON public.purchase_voids AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Admins can read purchase voids" ON public.purchase_voids AS PERMISSIVE FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY push_subscriptions_own ON public.push_subscriptions AS PERMISSIVE FOR ALL TO public USING ((auth.uid() = user_id));

CREATE POLICY quick_entries_admin_only ON public.quick_entries AS PERMISSIVE FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY route_tolls_delete_admin ON public.route_tolls AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY route_tolls_insert_admin ON public.route_tolls AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY route_tolls_select_auth ON public.route_tolls AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY route_tolls_update_admin ON public.route_tolls AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY routes_delete_admin ON public.routes AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY routes_insert_admin ON public.routes AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY routes_select_auth ON public.routes AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY routes_update_admin ON public.routes AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY saved_locations_delete ON public.saved_locations AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY saved_locations_read ON public.saved_locations AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY saved_locations_update ON public.saved_locations AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY saved_locations_write ON public.saved_locations AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY scheduled_payments_delete_admin ON public.scheduled_payments AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY scheduled_payments_insert_admin ON public.scheduled_payments AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY scheduled_payments_select_auth ON public.scheduled_payments AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY scheduled_payments_update_admin ON public.scheduled_payments AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY service_cash_receipts_delete ON public.service_cash_receipts AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin_user_safe() OR (is_operator_user_safe() AND (created_by = auth.uid()))));

CREATE POLICY service_cash_receipts_insert ON public.service_cash_receipts AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (((created_by = auth.uid()) AND (is_admin_user_safe() OR is_operator_user_safe())));

CREATE POLICY service_cash_receipts_select_scoped ON public.service_cash_receipts AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe() OR (is_client_user_safe() AND (EXISTS ( SELECT 1
   FROM services s
  WHERE ((s.id = service_cash_receipts.service_id) AND (s.client_id = get_user_client_id_safe())))))));

CREATE POLICY service_cash_receipts_update ON public.service_cash_receipts AS PERMISSIVE FOR UPDATE TO authenticated USING ((created_by = auth.uid())) WITH CHECK ((created_by = auth.uid()));

CREATE POLICY "Authenticated users can insert" ON public.service_change_history AS PERMISSIVE FOR INSERT TO public WITH CHECK ((auth.uid() IS NOT NULL));

CREATE POLICY service_change_history_scoped_select ON public.service_change_history AS PERMISSIVE FOR SELECT TO authenticated USING ((has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'operator'::app_role) OR (EXISTS ( SELECT 1
   FROM services s
  WHERE ((s.id = service_change_history.service_id) AND (s.client_id = get_user_client_id_safe()))))));

CREATE POLICY service_closures_delete ON public.service_closures AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY service_closures_read ON public.service_closures AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY service_closures_update ON public.service_closures AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY service_closures_write ON public.service_closures AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY service_costs_delete_admin ON public.service_costs AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY service_costs_insert_staff ON public.service_costs AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY service_costs_select_staff ON public.service_costs AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY service_costs_update_staff ON public.service_costs AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY service_rates_admin_full ON public.service_rates AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY service_rates_client_own ON public.service_rates AS PERMISSIVE FOR SELECT TO authenticated USING ((is_client_user_safe() AND (client_id = get_user_client_id_safe())));

CREATE POLICY service_rates_operator_read ON public.service_rates AS PERMISSIVE FOR SELECT TO authenticated USING (is_operator_user_safe());

CREATE POLICY service_resources_read ON public.service_resources AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY service_resources_write ON public.service_resources AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY service_types_delete_admin ON public.service_types AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY service_types_insert_admin ON public.service_types AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY service_types_select_auth ON public.service_types AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY service_types_update_admin ON public.service_types AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY service_update_error_logs_admin_only ON public.service_update_error_logs AS PERMISSIVE FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY services_admin_full_access ON public.services AS PERMISSIVE FOR ALL TO authenticated USING (is_admin_user_safe());

CREATE POLICY services_client_own_data ON public.services AS PERMISSIVE FOR SELECT TO authenticated USING ((is_client_user_safe() AND (client_id = get_user_client_id_safe())));

CREATE POLICY services_operator_insert ON public.services AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_operator_user_safe());

CREATE POLICY services_operator_select_scoped ON public.services AS PERMISSIVE FOR SELECT TO authenticated USING ((is_operator_user_safe() AND (EXISTS ( SELECT 1
   FROM operators o
...
CREATE POLICY services_update_scoped ON public.services AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin_user_safe() OR (is_operator_user_safe() AND (EXISTS ( SELECT 1
   FROM operators o
  WHERE ((o.user_id = auth.uid()) AND ((o.id = services.operator_id) OR (EXISTS ( SELECT 1
           FROM service_resources sr
          WHERE ((sr.service_id = services.id) AND (sr.operator_id = o.id))))))))))) WITH CHECK ((is_admin_user_safe() OR (is_operator_user_safe() AND (EXISTS ( SELECT 1
   FROM operators o
  WHERE ((o.user_id = auth.uid()) AND ((o.id = services.operator_id) OR (EXISTS ( SELECT 1
           FROM service_resources sr
          WHERE ((sr.service_id = services.id) AND (sr.operator_id = o.id)))))))))));

CREATE POLICY services_viewer_read_access ON public.services AS PERMISSIVE FOR SELECT TO authenticated USING (has_role(auth.uid(), 'viewer'::app_role));

CREATE POLICY supplier_categories_delete ON public.supplier_categories AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY supplier_categories_read ON public.supplier_categories AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY supplier_categories_update ON public.supplier_categories AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY supplier_categories_write ON public.supplier_categories AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY supplier_invoice_items_delete_admin ON public.supplier_invoice_items AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY supplier_invoice_items_insert_admin ON public.supplier_invoice_items AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY supplier_invoice_items_select_auth ON public.supplier_invoice_items AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY supplier_invoice_items_update_admin ON public.supplier_invoice_items AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY supplier_invoices_delete_admin ON public.supplier_invoices AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY supplier_invoices_insert_admin ON public.supplier_invoices AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY supplier_invoices_select_auth ON public.supplier_invoices AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY supplier_invoices_update_admin ON public.supplier_invoices AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY supplier_payments_delete_admin ON public.supplier_payments AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY supplier_payments_insert_admin ON public.supplier_payments AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY supplier_payments_select_auth ON public.supplier_payments AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY supplier_payments_update_admin ON public.supplier_payments AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY "Usuarios autenticados pueden ver proveedores" ON public.suppliers AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY suppliers_delete_admin ON public.suppliers AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY suppliers_insert_admin ON public.suppliers AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY suppliers_update_admin ON public.suppliers AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY system_settings_admin_only ON public.system_settings AS PERMISSIVE FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY toll_rates_delete_admin ON public.toll_rates AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY toll_rates_insert_admin ON public.toll_rates AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY toll_rates_select_auth ON public.toll_rates AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY toll_rates_update_admin ON public.toll_rates AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY toll_stations_delete_admin ON public.toll_stations AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY toll_stations_insert_admin ON public.toll_stations AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_admin_user_safe());

CREATE POLICY toll_stations_select_auth ON public.toll_stations AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY toll_stations_update_admin ON public.toll_stations AS PERMISSIVE FOR UPDATE TO authenticated USING (is_admin_user_safe()) WITH CHECK (is_admin_user_safe());

CREATE POLICY trip_estimates_delete ON public.trip_estimates AS PERMISSIVE FOR DELETE TO authenticated USING (is_admin_user_safe());

CREATE POLICY trip_estimates_read ON public.trip_estimates AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY trip_estimates_update ON public.trip_estimates AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY trip_estimates_write ON public.trip_estimates AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY user_activity_log_insert_own ON public.user_activity_log AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((auth.uid() = user_id));

CREATE POLICY user_activity_log_select_admin ON public.user_activity_log AS PERMISSIVE FOR SELECT TO authenticated USING (is_admin_user_safe());

CREATE POLICY user_invitations_admin_only ON public.user_invitations AS PERMISSIVE FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Admins can manage all permissions" ON public.user_module_permissions AS PERMISSIVE FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Users can view own permissions" ON public.user_module_permissions AS PERMISSIVE FOR SELECT TO authenticated USING ((user_id = auth.uid()));

CREATE POLICY admin_only_role_management ON public.user_roles AS PERMISSIVE FOR ALL TO public USING (is_admin_user(auth.uid())) WITH CHECK (is_admin_user(auth.uid()));

CREATE POLICY users_view_own_role ON public.user_roles AS PERMISSIVE FOR SELECT TO public USING ((user_id = auth.uid()));

CREATE POLICY "Users can create their own settings" ON public.user_settings AS PERMISSIVE FOR INSERT TO public WITH CHECK ((auth.uid() = user_id));
...
CREATE POLICY auth_read_cache ON public.vehicle_api_cache AS PERMISSIVE FOR SELECT TO public USING ((auth.role() = 'authenticated'::text));

CREATE POLICY vehicle_brands_delete_staff ON public.vehicle_brands AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY vehicle_brands_insert_staff ON public.vehicle_brands AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY vehicle_brands_select_auth ON public.vehicle_brands AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY vehicle_brands_update_staff ON public.vehicle_brands AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY vehicle_models_delete_staff ON public.vehicle_models AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY vehicle_models_insert_staff ON public.vehicle_models AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY vehicle_models_select_auth ON public.vehicle_models AS PERMISSIVE FOR SELECT TO authenticated USING (true);

CREATE POLICY vehicle_models_update_staff ON public.vehicle_models AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin_user_safe() OR is_operator_user_safe())) WITH CHECK ((is_admin_user_safe() OR is_operator_user_safe()));

CREATE POLICY "Admins can view whatsapp alert dedupe" ON public.whatsapp_alert_dedupe AS PERMISSIVE FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Admins can hide whatsapp logs" ON public.whatsapp_message_log AS PERMISSIVE FOR UPDATE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Admins can view whatsapp logs" ON public.whatsapp_message_log AS PERMISSIVE FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Admins can manage whatsapp settings" ON public.whatsapp_settings AS PERMISSIVE FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

-- ===== GRANTS (tablas/secuencias, roles API) =====
...
GRANT ALL ON public.backup_email_config TO anon;

GRANT ALL ON public.backup_email_config TO authenticated;

GRANT ALL ON public.backup_email_config TO service_role;

GRANT ALL ON public.backup_logs TO anon;

GRANT ALL ON public.backup_logs TO authenticated;
...
GRANT ALL ON public.closure_services TO anon;

GRANT ALL ON public.closure_services TO authenticated;

GRANT ALL ON public.closure_services TO service_role;

GRANT ALL ON public.company_data TO anon;

GRANT ALL ON public.company_data TO authenticated;

GRANT ALL ON public.company_data TO service_role;

GRANT ALL ON public.company_profiles TO anon;

GRANT ALL ON public.company_profiles TO authenticated;

GRANT ALL ON public.company_profiles TO service_role;

GRANT ALL ON public.cost_bulk_payment_operations TO anon;

GRANT ALL ON public.cost_bulk_payment_operations TO authenticated;

GRANT ALL ON public.cost_bulk_payment_operations TO service_role;

GRANT ALL ON public.cost_categories TO anon;

GRANT ALL ON public.cost_categories TO authenticated;

GRANT ALL ON public.cost_categories TO service_role;

GRANT ALL ON public.cost_centers TO anon;

GRANT ALL ON public.cost_centers TO authenticated;

GRANT ALL ON public.cost_centers TO service_role;

GRANT ALL ON public.cost_change_history TO anon;

GRANT ALL ON public.cost_change_history TO authenticated;

GRANT ALL ON public.cost_change_history TO service_role;

GRANT ALL ON public.cost_inventory_items TO anon;

GRANT ALL ON public.cost_inventory_items TO authenticated;

GRANT ALL ON public.cost_inventory_items TO service_role;

GRANT ALL ON public.cost_subcategories TO anon;

GRANT ALL ON public.cost_subcategories TO authenticated;

GRANT ALL ON public.cost_subcategories TO service_role;

GRANT ALL ON public.costs TO anon;

GRANT ALL ON public.costs TO authenticated;

GRANT ALL ON public.costs TO service_role;

GRANT ALL ON public.crane_consumption_rates TO anon;

GRANT ALL ON public.crane_consumption_rates TO authenticated;

GRANT ALL ON public.crane_consumption_rates TO service_role;

GRANT ALL ON public.crane_documents TO anon;

GRANT ALL ON public.crane_documents TO authenticated;

GRANT ALL ON public.crane_documents TO service_role;

GRANT ALL ON public.crane_maintenance TO anon;

GRANT ALL ON public.crane_maintenance TO authenticated;

GRANT ALL ON public.crane_maintenance TO service_role;

GRANT ALL ON public.crane_part_change_history TO anon;

GRANT ALL ON public.crane_part_change_history TO authenticated;

GRANT ALL ON public.crane_part_change_history TO service_role;

GRANT ALL ON public.crane_parts TO anon;

GRANT ALL ON public.crane_parts TO authenticated;

GRANT ALL ON public.crane_parts TO service_role;

GRANT ALL ON public.cranes TO anon;

GRANT ALL ON public.cranes TO authenticated;

GRANT ALL ON public.cranes TO service_role;

GRANT ALL ON public.creditors TO anon;

GRANT ALL ON public.creditors TO authenticated;

GRANT ALL ON public.creditors TO service_role;

GRANT ALL ON public.debt_installments TO anon;

GRANT ALL ON public.debt_installments TO authenticated;

GRANT ALL ON public.debt_installments TO service_role;

GRANT ALL ON public.debt_payments TO anon;
...
GRANT ALL ON public.document_alerts TO anon;

GRANT ALL ON public.document_alerts TO authenticated;

GRANT ALL ON public.document_alerts TO service_role;

GRANT ALL ON public.frontend_error_logs TO anon;

GRANT ALL ON public.frontend_error_logs TO authenticated;

GRANT ALL ON public.frontend_error_logs TO service_role;

GRANT ALL ON public.fuel_prices TO anon;

GRANT ALL ON public.fuel_prices TO authenticated;

GRANT ALL ON public.fuel_prices TO service_role;

GRANT ALL ON public.import_batch_records TO anon;

GRANT ALL ON public.import_batch_records TO authenticated;

GRANT ALL ON public.import_batch_records TO service_role;
...
GRANT ALL ON public.import_history_log TO anon;

GRANT ALL ON public.import_history_log TO authenticated;

GRANT ALL ON public.import_history_log TO service_role;

GRANT ALL ON public.import_rut_mappings TO anon;

GRANT ALL ON public.import_rut_mappings TO authenticated;

GRANT ALL ON public.import_rut_mappings TO service_role;

GRANT ALL ON public.income_categories TO anon;

GRANT ALL ON public.income_categories TO authenticated;

GRANT ALL ON public.income_categories TO service_role;

GRANT ALL ON public.income_subcategories TO anon;

GRANT ALL ON public.income_subcategories TO authenticated;

GRANT ALL ON public.income_subcategories TO service_role;
...
GRANT ALL ON public.inspections TO anon;

GRANT ALL ON public.inspections TO authenticated;

GRANT ALL ON public.inspections TO service_role;

GRANT ALL ON public.internal_scheduler_secrets TO anon;

GRANT ALL ON public.internal_scheduler_secrets TO authenticated;

GRANT ALL ON public.internal_scheduler_secrets TO service_role;

GRANT ALL ON public.inventory_alerts TO anon;

GRANT ALL ON public.inventory_alerts TO authenticated;

GRANT ALL ON public.inventory_alerts TO service_role;

GRANT ALL ON public.inventory_categories TO anon;

GRANT ALL ON public.inventory_categories TO authenticated;

GRANT ALL ON public.inventory_categories TO service_role;

GRANT ALL ON public.inventory_consumptions TO anon;

GRANT ALL ON public.inventory_consumptions TO authenticated;

GRANT ALL ON public.inventory_consumptions TO service_role;
...
GRANT ALL ON public.inventory_locations TO authenticated;

GRANT ALL ON public.inventory_locations TO service_role;

GRANT ALL ON public.inventory_movement_change_history TO anon;

GRANT ALL ON public.inventory_movement_change_history TO authenticated;

GRANT ALL ON public.inventory_movement_change_history TO service_role;
...
GRANT ALL ON public.inventory_stock TO anon;

GRANT ALL ON public.inventory_stock TO authenticated;

GRANT ALL ON public.inventory_stock TO service_role;

GRANT ALL ON public.inventory_suppliers TO anon;

GRANT ALL ON public.inventory_suppliers TO authenticated;

GRANT ALL ON public.inventory_suppliers TO service_role;

GRANT ALL ON public.invoice_alert_settings TO anon;

GRANT ALL ON public.invoice_alert_settings TO authenticated;

GRANT ALL ON public.invoice_alert_settings TO service_role;

GRANT ALL ON public.invoice_cancellations TO anon;

GRANT ALL ON public.invoice_cancellations TO authenticated;

GRANT ALL ON public.invoice_cancellations TO service_role;

GRANT ALL ON public.invoice_closures TO anon;

GRANT ALL ON public.invoice_closures TO authenticated;

GRANT ALL ON public.invoice_closures TO service_role;

GRANT ALL ON public.invoice_services TO anon;

GRANT ALL ON public.invoice_services TO authenticated;
...
GRANT ALL ON public.operator_documents TO anon;

GRANT ALL ON public.operator_documents TO authenticated;
...
GRANT ALL ON public.orphan_crane_parts_candidates TO anon;

GRANT ALL ON public.orphan_crane_parts_candidates TO authenticated;

GRANT ALL ON public.orphan_crane_parts_candidates TO service_role;

GRANT ALL ON public.password_reset_rate_limits TO anon;

GRANT ALL ON public.password_reset_rate_limits TO authenticated;

GRANT ALL ON public.password_reset_rate_limits TO service_role;

GRANT ALL ON public.patent_search_history TO anon;

GRANT ALL ON public.patent_search_history TO authenticated;

GRANT ALL ON public.patent_search_history TO service_role;

GRANT ALL ON public.payment_applications TO anon;

GRANT ALL ON public.payment_applications TO authenticated;

GRANT ALL ON public.payment_applications TO service_role;

GRANT ALL ON public.payment_terms TO anon;

GRANT ALL ON public.payment_terms TO authenticated;
...
GRANT ALL ON public.purchase_voids TO anon;

GRANT ALL ON public.purchase_voids TO authenticated;

GRANT ALL ON public.purchase_voids TO service_role;
...
GRANT ALL ON public.quick_entries TO anon;

GRANT ALL ON public.quick_entries TO authenticated;

GRANT ALL ON public.quick_entries TO service_role;

GRANT ALL ON public.route_tolls TO anon;

GRANT ALL ON public.route_tolls TO authenticated;

GRANT ALL ON public.route_tolls TO service_role;
...
GRANT ALL ON public.saved_locations TO anon;

GRANT ALL ON public.saved_locations TO authenticated;

GRANT ALL ON public.saved_locations TO service_role;

GRANT ALL ON public.scheduled_payments TO anon;

GRANT ALL ON public.scheduled_payments TO authenticated;

GRANT ALL ON public.scheduled_payments TO service_role;

GRANT ALL ON public.service_cash_receipts TO anon;

GRANT ALL ON public.service_cash_receipts TO authenticated;

GRANT ALL ON public.service_cash_receipts TO service_role;

GRANT ALL ON public.service_change_history TO anon;

GRANT ALL ON public.service_change_history TO authenticated;

GRANT ALL ON public.service_change_history TO service_role;

GRANT ALL ON public.service_closures TO anon;

GRANT ALL ON public.service_closures TO authenticated;

GRANT ALL ON public.service_closures TO service_role;

GRANT ALL ON public.service_costs TO anon;

GRANT ALL ON public.service_costs TO authenticated;

GRANT ALL ON public.service_costs TO service_role;

GRANT ALL ON public.service_rates TO anon;

GRANT ALL ON public.service_rates TO authenticated;

GRANT ALL ON public.service_rates TO service_role;

GRANT ALL ON public.service_resources TO anon;

GRANT ALL ON public.service_resources TO authenticated;

GRANT ALL ON public.service_resources TO service_role;

GRANT ALL ON public.service_types TO anon;

GRANT ALL ON public.service_types TO authenticated;

GRANT ALL ON public.service_types TO service_role;

GRANT ALL ON public.service_update_error_logs TO anon;

GRANT ALL ON public.service_update_error_logs TO authenticated;

GRANT ALL ON public.service_update_error_logs TO service_role;
...
GRANT ALL ON public.services_with_excess_summary TO anon;

GRANT ALL ON public.services_with_excess_summary TO authenticated;

GRANT ALL ON public.services_with_excess_summary TO service_role;

GRANT ALL ON public.supplier_categories TO anon;

GRANT ALL ON public.supplier_categories TO authenticated;

GRANT ALL ON public.supplier_categories TO service_role;

GRANT ALL ON public.supplier_invoice_items TO anon;

GRANT ALL ON public.supplier_invoice_items TO authenticated;
...
GRANT ALL ON public.toll_rates TO anon;

GRANT ALL ON public.toll_rates TO authenticated;

GRANT ALL ON public.toll_rates TO service_role;

GRANT ALL ON public.toll_stations TO anon;

GRANT ALL ON public.toll_stations TO authenticated;

GRANT ALL ON public.toll_stations TO service_role;

GRANT ALL ON public.trip_estimates TO anon;

GRANT ALL ON public.trip_estimates TO authenticated;

GRANT ALL ON public.trip_estimates TO service_role;

GRANT ALL ON public.user_activity_log TO anon;

GRANT ALL ON public.user_activity_log TO authenticated;

GRANT ALL ON public.user_activity_log TO service_role;

GRANT ALL ON public.user_invitations TO anon;

GRANT ALL ON public.user_invitations TO authenticated;

GRANT ALL ON public.user_invitations TO service_role;

GRANT ALL ON public.user_module_permissions TO anon;

GRANT ALL ON public.user_module_permissions TO authenticated;
...
GRANT ALL ON public.vehicle_api_cache TO anon;

GRANT ALL ON public.vehicle_api_cache TO authenticated;

GRANT ALL ON public.vehicle_api_cache TO service_role;

GRANT ALL ON public.vehicle_brands TO anon;

GRANT ALL ON public.vehicle_brands TO authenticated;
...
GRANT ALL ON public.whatsapp_alert_dedupe TO anon;

GRANT ALL ON public.whatsapp_alert_dedupe TO authenticated;

GRANT ALL ON public.whatsapp_alert_dedupe TO service_role;

GRANT ALL ON public.whatsapp_message_log TO anon;

GRANT ALL ON public.whatsapp_message_log TO authenticated;

GRANT ALL ON public.whatsapp_message_log TO service_role;

GRANT ALL ON public.whatsapp_settings TO anon;
