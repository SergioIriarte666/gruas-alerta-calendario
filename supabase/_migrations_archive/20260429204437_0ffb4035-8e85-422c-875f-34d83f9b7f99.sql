...
-- Reemplazar la búsqueda con normalización (tildes/case) y multi-palabra
DROP FUNCTION IF EXISTS public.search_voidable_inventory_purchases(text);
...
BEGIN
  ...
  enriched AS (
    SELECT
      b.*,
      s.name AS sup_name,
      -- agregamos textos de items de factura
      (
      ...
      -- agregamos textos de movimientos + nombres de items
      (
        SELECT string_agg(
          coalesce(ii.name, '') || ' ' ||
  ...
  filtered AS (
    ...
    WHERE
      v_term IS NULL
      OR (
        -- todas las palabras deben aparecer en el haystack
        (
