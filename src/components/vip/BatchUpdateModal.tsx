...
import { FileText, ShoppingCart, Check, X, AlertCircle, Hash, Calendar, ToggleLeft, ToggleRight, RefreshCw, ArrowRight } from 'lucide-react';
...
export const BatchUpdateModal: React.FC<BatchUpdateModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const typesText = activeTypes.length === 2 ? 'cotizaciones y órdenes de compra' : activeTypes[0] === 'quote' ? 'cotizaciones' : 'órdenes de compra';
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl h-[75vh] bg-card border select-none flex flex-col p-0 vip-pipeline-scope" style={{
      position: 'fixed',
      top: '50%',
      left: '50%',
      ...
      userSelect: 'none',
      pointerEvents: 'auto'
    }} onMouseDown={e => e.stopPropagation()} onDragStart={e => e.preventDefault()} draggable={false}>
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                <Badge variant="outline" className="text-foreground border-blue-500/30 bg-inherit">
              ...
              <div className="h-20 overflow-y-auto space-y-2 border bg-card p-3 rounded-md">
                <div className="space-y-2">
                  {selectedServices.map((service, index) => <div key={service.id} className="flex items-center justify-between text-sm">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-4">
                ...
                {enableQuote && <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <Label htmlFor="quote-prefix" className="text-foreground">Prefijo</Label>
                        <Input id="quote-prefix" value={batchData.quote.prefix} onChange={e => updateData('quote', 'prefix', e.target.value)} placeholder="COT-" className="bg-background border" />
                      </div>
                      
                      <div>
                        <Label htmlFor="quote-base" className="text-foreground">Número base (mismo para todos)</Label>
                        <Input id="quote-base" value={batchData.quote.baseNumber} onChange={e => {
                      updateData('quote', 'baseNumber', e.target.value);
                      if (e.target.value) updateData('quote', 'startingNumber', '');
                    }} placeholder="2024001" className="bg-background border" />
                      </div>
                      
                      <div className="col-span-2">
                        <Label htmlFor="quote-start" className="text-foreground">Número inicial (secuencial)</Label>
                        <Input id="quote-start" type="number" value={batchData.quote.startingNumber} onChange={e => {
                      updateData('quote', 'startingNumber', e.target.value);
                      if (e.target.value) updateData('quote', 'baseNumber', '');
                    }} placeholder="1001" className="bg-background border" disabled={!!batchData.quote.baseNumber} />
                        {batchData.quote.startingNumber && <p className="text-xs text-muted-foreground mt-1">
              ...
              <div className="space-y-4">
                ...
                {enablePurchaseOrder && <div className="p-3 bg-green-500/10 border border-green-500/20 rounded-lg">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <Label htmlFor="po-prefix" className="text-foreground">Prefijo</Label>
                        <Input id="po-prefix" value={batchData.purchase_order.prefix} onChange={e => updateData('purchase_order', 'prefix', e.target.value)} placeholder="OC-" className="bg-background border" />
                      </div>
                      
                      <div>
                        <Label htmlFor="po-base" className="text-foreground">Número base (mismo para todos)</Label>
                        <Input id="po-base" value={batchData.purchase_order.baseNumber} onChange={e => {
                      ...
                      <div className="col-span-2">
                        <Label htmlFor="po-start" className="text-foreground">Número inicial (secuencial)</Label>
                        <Input id="po-start" type="number" value={batchData.purchase_order.startingNumber} onChange={e => {
                      updateData('purchase_order', 'startingNumber', e.target.value);
                      if (e.target.value) updateData('purchase_order', 'baseNumber', '');
                    }} placeholder="1001" className="bg-background border" disabled={!!batchData.purchase_order.baseNumber} />
                        {batchData.purchase_order.startingNumber && <p className="text-xs text-muted-foreground mt-1">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-medium text-foreground">Gestión Automática de Estado</h3>
                </div>
                <Switch checked={autoUpdateStatus} onCheckedChange={setAutoUpdateStatus} />
              </div>
              
              {autoUpdateStatus && (enableQuote || enablePurchaseOrder) && <div className="p-3 bg-purple-500/10 border border-purple-500/20 rounded-lg">
                  <div className="flex items-center gap-3 mb-2">
                    <AlertCircle className="w-4 h-4 text-purple-400" />
                    <span className="text-purple-300 font-medium">Estado Resultante</span>
                  </div>
                  <div className="text-sm text-foreground">
                    {enablePurchaseOrder && enableQuote ? <div className="flex items-center gap-2">
                      ...
                      </div> : enablePurchaseOrder ? <div className="flex items-center gap-2">
                        <span>Los servicios cambiarán automáticamente a</span>
                        <Badge className="bg-green-500/20 text-foreground border-green-500/30">
                          Con Orden de Compra
                        </Badge>
                      </div> : enableQuote ? <div className="flex items-center gap-2">
                        <span>Los servicios cambiarán automáticamente a</span>
            ...
            {/* Preview */}
            {preview.length > 0 && <div className="p-4 bg-muted/50 border rounded-lg">
                <h4 className="text-foreground font-medium mb-3 flex items-center gap-2">
                  <AlertCircle className="w-4 h-4 text-blue-400" />
                  Vista Previa
                </h4>
                <div className="space-y-2">
                  {preview.map((item, index) => <div key={index} className="bg-card p-3 rounded border">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-foreground font-medium">{item.folio}</span>
                        <div className="flex items-center gap-2">
                          {item.quote && <Badge variant="outline" className="bg-blue-500/20 text-blue-300 border-blue-500/30">
                              COT: {item.quote}
                            </Badge>}
                          {item.purchaseOrder && <Badge variant="outline" className="bg-green-500/20 text-green-300 border-green-500/30">
                              O.C.: {item.purchaseOrder}
                            </Badge>}
                        </div>
                      </div>
                      
                      {autoUpdateStatus && item.targetStatus && <div className="flex items-center gap-2 text-xs">
                          <Badge variant="secondary" className="bg-muted text-muted-foreground">
                            {item.currentStatus === 'new' ? 'Nuevo' : item.currentStatus === 'quoted' ? 'Cotizado' : item.currentStatus === 'with_purchase_order' ? 'Con O.C.' : item.currentStatus}
                          </Badge>
                  ...
                  {selectedServices.length > 3 && <div className="text-center text-muted-foreground text-sm">
            ...
            <div>
              ...
              <Textarea id="notes" value={enableQuote ? batchData.quote.notes : batchData.purchase_order.notes} onChange={e => {
            ...
            }} placeholder="Notas adicionales sobre esta actualización por lotes..." className="bg-background border" rows={3} />
            </div>
          </div>
        </div>
        ...
        <DialogFooter className="flex-shrink-0 p-6 border-t">
          <div className="flex justify-end gap-3 w-full">
            ...
            <Button onClick={handleSubmit} disabled={isLoading || !enableQuote && !enablePurchaseOrder}>
