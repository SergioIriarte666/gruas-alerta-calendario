...
-- Trigger para ejecutar automáticamente la actualización de facturas vencidas
-- Se ejecutará cada vez que se consulten las facturas
CREATE OR REPLACE FUNCTION public.auto_update_overdue_invoices_trigger()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
  -- Ejecutar la actualización en background
  PERFORM public.update_overdue_invoices();
  RETURN NULL;
END;
$function$;

-- Crear trigger que se ejecute cuando se acceda a la tabla de facturas
DROP TRIGGER IF EXISTS auto_update_overdue_trigger ON public.invoices;
CREATE TRIGGER auto_update_overdue_trigger
  AFTER SELECT ON public.invoices
  FOR EACH STATEMENT
  EXECUTE FUNCTION public.auto_update_overdue_invoices_trigger();