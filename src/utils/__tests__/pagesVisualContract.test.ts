...
const pagesRoot = resolve(workspaceRoot, "src/pages");
...
const protectedFiles = collectSourceFiles(pagesRoot);
...
const malformedSemanticClass =
  /(?:(?:soft|muted|foreground|card|border|primary|info|success|warning|danger|auth)\d|\/\d+\/\d+)/g;

describe("contrato visual de las páginas", () => {
